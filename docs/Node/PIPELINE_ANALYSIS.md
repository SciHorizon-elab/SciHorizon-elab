...
## 0. Pipeline 入口 (Entry Point)

> Pipeline 入口: `scihorizon_elab/pipeline/create_task.py`
>
> Usage:
> ```bash
> # 基础用法
> python -m scihorizon_elab.pipeline.create_task "<instruction>"
>
> # 显式指定 series 文件名 (覆盖 Analyzer 推断)
...
**职责**：动态注册生成的任务类到 scihorizon_elab 任务系统。
