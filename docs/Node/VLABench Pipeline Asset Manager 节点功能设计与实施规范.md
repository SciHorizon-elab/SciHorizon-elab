# scihorizon_elab Pipeline: `Asset Manager` 节点功能设计与实施规范
...
| `class_name`             | String   | **动态推断出的 scihorizon_elab 底层类名**。决定了该实体能调用哪些 Mixin（如能否装水）。 | `"ChemistryBeaker"`, `"CommonContainer"`      |
...
| `scihorizon_elab/configs/constant.py` | 资产注册表：`name2class_xml`（开发者维护的权威注册表） |
| `scripts/scihorizon_elab/assets/asset_cache.json` | 缓存文件（Normalizer 写入，Asset Manager 下载后更新） |
