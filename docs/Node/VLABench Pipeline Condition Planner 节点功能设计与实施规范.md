# scihorizon_elab Pipeline: `Condition Planner` 节点功能设计与实施规范
...
| `condition_type` | String | Condition 类型（注册在 scihorizon_elab/tasks/condition.py） | `"is_grasped"`, `"contain"`, `"lift"` |
...
Condition Planner 支持以下 condition 类型（注册在 `scihorizon_elab/tasks/condition.py`）：
...
- **Condition 类定义**：`scihorizon_elab/tasks/condition.py`
- **Condition 注册机制**：`scihorizon_elab/utils/register.py`
