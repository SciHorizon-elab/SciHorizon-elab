# scihorizon_elab Pipeline: `Normalizer` 节点功能设计与实施规范

## 1. 节点定位与职责 (Node Overview)

`Normalizer` 节点是整个 scihorizon_elab Pipeline 的“语义到物理的网关（Semantic-to-Physical Gateway）”或“符号链接器（Symbol Linker）”。
