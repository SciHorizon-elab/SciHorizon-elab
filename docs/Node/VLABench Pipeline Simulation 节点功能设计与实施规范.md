# scihorizon_elab Pipeline: `Simulation` 节点功能设计与实施规范

## 1. 节点定位与职责 (Node Overview)

`Simulation` 节点是整个 scihorizon_elab Pipeline 的"物理仿真执行器（Physics Simulation Executor）"，也是 Pipeline 的最终验证关卡。
...
```
1. 设置 SCIHORIZON_ELAB_ROOT 环境变量
2. import scihorizon_elab.robots / tasks（触发 @register）
...
`simulation_node` 在标准 `importlib.import_module("scihorizon_elab.benchmark.tasks.autogen_tasks.primitive")` 之外，额外动态加载 series 文件：

```python
series_path = $SCIHORIZON_ELAB_ROOT/tasks/autogen_tasks/primitive/<task_name>_series.py
...
### 9.1 SCIHORIZON_ELAB_ROOT 路径

必须设置为 scihorizon_elab **子目录**（含 `tasks/`、`assets/` 的那一层）：

```
SCIHORIZON_ELAB_ROOT = /path/to/scihorizon_elab/scihorizon_elab  # ✓ 正确
SCIHORIZON_ELAB_ROOT = /path/to/scihorizon_elab            # ✗ 错误（少一层）
```
