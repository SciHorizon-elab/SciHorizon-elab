...
└─────────────────────────────────────────────────────────────────────────┘
                                    ...
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  Terminal 2: scihorizon_elab Evaluator Client (vlabench_2 conda)               │
│                                                                         │
...
- **客户端**需要 scihorizon_elab 的依赖（MuJoCo、dm_control 等），管理仿真环境
...
```bash
cd /ssd/qinmaokai/workspace/SciHorizon-ELAB/third_party/openpi
/opt/miniconda3/bin/uv venv --python 3.11 examples/vlabench/.venv
source examples/vlabench/.venv/bin/activate
/opt/miniconda3/bin/uv pip sync examples/vlabench/requirements.txt
/opt/miniconda3/bin/uv pip install -e packages/openpi-client
/opt/miniconda3/bin/uv pip install -e /ssd/qinmaokai/workspace/SciHorizon-ELAB
```

#### 环境 2：scihorizon_elab Conda 环境
...
模型权重下载到：
```
/ssd/qinmaokai/.cache/openpi/vlabench_checkpoints/pi0_fast_primitive_10task/
```
...
```
/ssd/qinmaokai/.cache/openpi/big_vision/paligemma_tokenizer.model
```
...
cp /tmp/tokenizer/tokenizer.model /ssd/qinmaokai/.cache/openpi/big_vision/paligemma_tokenizer.model
mkdir -p /ssd/qinmaokai/.cache/openpi/big_vision
```
...
```bash
# 1. 激活 OpenPI 虚拟环境
source /ssd/qinmaokai/workspace/SciHorizon-ELAB/third_party/openpi/examples/vlabench/.venv/bin/activate

# 2. 设置环境变量
export PYTHONPATH=/ssd/qinmaokai/workspace/SciHorizon-ELAB/third_party/openpi/src:/ssd/qinmaokai/workspace/SciHorizon-ELAB:$PYTHONPATH
...
python scripts/serve_policy.py --port 8000 policy:checkpoint \
    ...
    --policy.dir=/ssd/qinmaokai/.cache/openpi/vlabench_checkpoints/pi0_fast_primitive_10task
```

**预期输出**：
```
INFO:root:Loading model...
INFO:absl:Finished restoring checkpoint from /ssd/qinmaokai/.cache/openpi/vlabench_checkpoints/pi0_fast_primitive_10task/params.
...
# 3. 进入 scihorizon_elab 项目根目录
cd /ssd/qinmaokai/workspace/SciHorizon-ELAB
...
[DEBUG] 所有模块导入成功
[DEBUG] main() 函数开始
[DEBUG] SCIHORIZON_ELAB_ROOT=/ssd/qinmaokai/workspace/SciHorizon-ELAB/scihorizon_elab
...
`/ssd/qinmaokai/workspace/SciHorizon-ELAB/scihorizon_elab/tasks/autogen_tasks/` 目录用于存放自动生成的任务。与 `scihorizon_elab/benchmark/tasks/autogen_tasks/primitive/` 下的手写任务不同，autogen 任务由流水线生成。

### 6.1 目录结构

```
scihorizon_elab/tasks/autogen_tasks/
├── __init__.py                      # 空白，仅占位
...
_autogen_dir = os.path.join(
    os.environ.get("SCIHORIZON_ELAB_ROOT", _VLABENCH_ROOT),
    "tasks", "autogen_tasks"
)
...
**关键点**：
- 必须先 `from scihorizon_elab.tasks import *`（触发 `register` 模块初始化），再做动态导入
- autogen 任务**不需要**在 `autogen_tasks/__init__.py` 中显式导入
- `scihorizon_elab/tasks/__init__.py` 中也不需要添加 `from scihorizon_elab.tasks.autogen_tasks import *`
...
`scihorizon_elab/tasks/autogen_tasks/lift_new_object_series.py`：
...
from scihorizon_elab.tasks.dm_task import *
from scihorizon_elab.tasks.autogen_tasks.base import PrimitiveTask
from scihorizon_elab.tasks.config_manager import BenchTaskConfigManager
from scihorizon_elab.utils.register import register
from scihorizon_elab.configs.constant import name2class_xml
...
`scihorizon_elab/configs/__init__.py`：
...
| 任务加载 | 静态导入 `scihorizon_elab.tasks` | 静态 + `autogen_tasks` 动态 glob |
...
def predict(self, obs, **kwargs):
    ...
    target_pos += np.array([0, -0.4, 0.78])  # 转换回 scihorizon_elab 坐标系
    return target_pos, target_euler, gripper_state
```
...
### 9.1 Condition 类（`scihorizon_elab/tasks/condition.py`）
...
**解决**：参考 [`TROUBLESHOOTING.md` 第 3 节](TROUBLESHOOTING.md#3-liftcondition-在-record_initial_state-未调用时误判为已满足)。`LiftCondition.is_met()` 已修复（`scihorizon_elab/tasks/condition.py` 第 367-372 行）。

### 11.2 `TypeError: 'NoneType' object is not iterable`

**症状**：评测启动后立即报错。

**根因**：任务没有在 `scihorizon_elab/configs/__init__.py` 的 `name2config` 中注册，导致 `find_key_by_value(name2config, task_name)` 返回 `None`。
...
**根因**：`autogen_tasks` 动态加载依赖于 `from scihorizon_elab.tasks import *` 已经执行过。检查导入顺序。

**解决**：确认 `evaluate_openpi.py` 中：
```python
from scihorizon_elab.robots import *       # 先 import scihorizon_elab.tasks 触发 register 初始化
from scihorizon_elab.tasks import *         # ← 这一步必须在前
...
PORT=8000
GPU_ID=1
CHECKPOINT=/ssd/qinmaokai/.cache/openpi/vlabench_checkpoints/pi0_fast_primitive_10task
SAVE_DIR=/ssd/qinmaokai/workspace/SciHorizon-ELAB/logs/openpi_eval
...
| `scihorizon_elab/configs/__init__.py` | `name2config` 任务名映射 |
| `scihorizon_elab/configs/task_config.json` | 任务基础配置（含 `evaluation.max_episode_length`） |
| `scihorizon_elab/configs/constant.py` | 实体注册（`name2class_xml`） |
| `scihorizon_elab/envs/dm_env.py` | MuJoCo 仿真环境 (`LM4ManipDMEnv`) |
| `scihorizon_elab/tasks/dm_task.py` | 任务基类 (`LM4ManipBaseTask`) |
| `scihorizon_elab/benchmark/tasks/autogen_tasks/primitive/` | 手写 primitive 任务 |
| `scihorizon_elab/tasks/autogen_tasks/` | 自动生成的 primitive 任务 |
| `scihorizon_elab/tasks/autogen_tasks/base.py` | autogen 任务的 `PrimitiveTask` 基类 |
| `scihorizon_elab/tasks/condition.py` | 成功条件系统（`LiftCondition` 等） |
| `scihorizon_elab/utils/register.py` | 任务/实体/条件注册系统 |
