...
This guide covers how to run scihorizon_elab evaluation with OpenPI (Physical Intelligence) policy. The evaluation requires two terminals:
- **Terminal 1**: Run the OpenPI policy server (provides inference service)
- **Terminal 2**: Run the scihorizon_elab evaluation client (connects to server)
...
#### Environment 2: scihorizon_elab Conda Environment
...
# 5. Start the server
python scripts/serve_policy.py --port 8000 policy:checkpoint --policy.config=pifast_ft_vlabench_primitive --policy.dir=/ssd/qinmaokai/.cache/openpi/vlabench_checkpoints/pi0_fast_primitive_10task
```
...
```bash
# 1. Activate scihorizon_elab conda environment
...
python examples/vlabench/eval.py \
    ...
    --args.n_episode 1 \
    --args.save_dir /ssd/qinmaokai/workspace/SciHorizon-ELAB/logs/openpi_eval
...
source /ssd/qinmaokai/workspace/SciHorizon-ELAB/third_party/openpi/examples/vlabench/.venv/bin/activate
cd /ssd/qinmaokai/workspace/SciHorizon-ELAB/third_party/openpi
/opt/miniconda3/bin/uv pip install -e . --no-deps
```
...
**Problem:** scihorizon_elab dependencies are incomplete.

**Solution:**
```bash
source /ssd/qinmaokai/workspace/SciHorizon-ELAB/third_party/openpi/examples/vlabench/.venv/bin/activate
/opt/miniconda3/bin/uv pip install -e /ssd/qinmaokai/workspace/SciHorizon-ELAB/src/rrt-algorithms --no-deps
```
...
PORT=8000
GPU_ID=1
CHECKPOINT_DIR=/ssd/qinmaokai/.cache/openpi/vlabench_checkpoints/pi0_fast_primitive_10task
