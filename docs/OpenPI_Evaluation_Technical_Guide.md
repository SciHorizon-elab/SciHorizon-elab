...
└─────────────────────────────────────────────────────────────────────────┘
                                    ...
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  Terminal 2: scihorizon_elab Evaluator Client (vlabench_2 conda环境)           │
│                                                                         │
│  eval.py                                                                │
...
| `scihorizon_elab/evaluation/evaluator/base.py` | 评测引擎 | `Evaluator.evaluate()`, `evaluate_single_episode()` |
| `scihorizon_elab/envs/dm_env.py` | MuJoCo仿真环境 | `load_env()`, `LM4ManipDMEnv` 类 |
| `scihorizon_elab/tasks/dm_task.py` | 任务基类 | `LM4ManipBaseTask`, 实体加载, 条件判断 |
| `scihorizon_elab/benchmark/tasks/autogen_tasks/primitive/base.py` | 原子任务基类 | `PrimitiveTask`, `get_intention_score()` |
| `scihorizon_elab/benchmark/tasks/autogen_tasks/primitive/xxx_series.py` | 具体任务实现 | `get_instruction()`, `get_expert_skill_sequence()`, 成功条件 |
| `scihorizon_elab/configs/task_config.json` | 任务基础配置 | 场景, 物体清单, 容器配置 |
| `scihorizon_elab/configs/constant.py` | 全局实体注册 | `name2class_xml` 字典 |
| `scihorizon_elab/configs/__init__.py` | 任务名映射 | `name2config` 映射 series名→任务名 |
| `scihorizon_elab/utils/register.py` | 注册系统 | 全局注册表, `@register.add_task()`, `@register.add_config_manager()` |
| `scihorizon_elab/tasks/condition.py` | 成功条件系统 | `Condition`, `contain`, `distance` 等 |
| `scihorizon_elab/utils/skill_lib.py` | 脚本技能库 | `SkillLib.pick()`, `SkillLib.lift()` 等 |
...
## 10. scihorizon_elab 评测的其他 Policy 实现

| Policy | 文件 | 用途 |
|--------|------|------|
| `Pi0` | `third_party/openpi/examples/vlabench/eval.py` | **OpenPI主评测** |
| `OpenPiPolicy` | `scihorizon_elab/evaluation/model/policy/openpi.py` | 旧版策略（V1） |
| `OpenVLA` | `scihorizon_elab/evaluation/model/policy/openvla.py` | OpenVLA模型 |
| `Gr00tPolicy` | `scihorizon_elab/evaluation/model/policy/gr00t.py` | Gr00t模型 |
| `RandomPolicy` | `scihorizon_elab/evaluation/model/policy/base.py` | 随机基线 |
