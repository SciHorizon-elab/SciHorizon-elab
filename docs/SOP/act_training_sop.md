...
你是 scihorizon_elab 项目的 **ACT 模型训练与部署专家**。你的职责是将一个 scihorizon_elab 任务(从注册到已有任务列表中选取)转换为专家轨迹数据,再经过格式转换、单卡训练(优化后),最终部署为可推理的策略模型,并验证其在仿真环境中的成功率。
...
| **evaluate_policy.py** | `scripts/evaluate_policy.py` | 在 scihorizon_elab 仿真环境中评测策略 |
| **act.py** (wrapper) | `scihorizon_elab/evaluation/model/policy/act.py` | ACT 推理时序控制与坐标变换 |
...
适用于:为指定 scihorizon_elab 任务生成专家演示轨迹,输出为 HDF5 文件。

**前置条件**:
- 目标任务已注册在 `scihorizon_elab.configs.name2config` 或 `scihorizon_elab.utils.register`
...
> ⚠️ 任务名需带 `_series` 后缀(沿袭 train_act.py 约定),脚本会自动处理 series name 解析。
...
| `trajectory` | `(T, 8)` | float32 | scihorizon_elab 格式动作 `[pos3, euler3, gripper1, gripper2]` |
...
**② Action 格式 — scihorizon_elab 8D → LeRobot 7D**

| 格式 | Shape | 字段 |
|------|-------|------|
| scihorizon_elab | `(T, 8)` | `[pos3, euler3, gripper1, gripper2]` |
...
| scihorizon_elab index | 内容 | LeRobot feature |
|---------------|------|----------------|
...
适用于:将 Phase 3 保存的 checkpoint 加载到 scihorizon_elab 仿真环境,运行指定任务评测成功率。
...
| 方向 | 代码位置 | 操作 |
|------|---------|------|
| 训练时(world→robot) | `convert_to_lerobot_act.py` | `ee_pos -= robot_frame_pos` |
...
| 步骤 | 格式 | Shape | 说明 |
|------|------|-------|------|
| HDF5 原始 | scihorizon_elab 8D | `(T, 8)` | `[pos3, euler3, gripper1, gripper2]` |
...
- scihorizon_elab 4 相机:`[0, 1, 2, 3]`
- ACT 使用:`[2]` = front (前视) + `[3]` = wrist (腕部)
- `convert_to_lerobot_act.py` 中:`images[i][2]` → `observation.image`,`images[i][3]` → `observation.wrist_image`
...
SCIHORIZON_ELAB_ROOT="/ssd/qinmaokai/workspace/SciVLABench"
LEROBOT_ROOT="/ssd/qinmaokai/workspace/lerobot"
HDF5_DIR="${SCIHORIZON_ELAB_ROOT}/dataset/training_data"

# ============================================================
# Phase 1: 生成专家轨迹数据 (HDF5)
# ============================================================
echo "=== Phase 1: 生成 ${N_EPISODES} 条轨迹 ==="
cd ${SCIHORIZON_ELAB_ROOT}
...
## 附录 A: scihorizon_elab 任务注册结构(参考)

新增自定义任务时,需在以下位置注册:

| 文件 | 路径 | 内容 |
|------|------|------|
| ConfigManager | `scihorizon_elab/tasks/autogen_tasks/primitive/<task_name>_series.py` | `load_objects()`、`get_instruction()`、`get_condition_config()` |
...
| constant.py | `scihorizon_elab/configs/constant.py` | `name2class_xml` 中注册模型 XML |

**参考示例**:`scihorizon_elab/tasks/autogen_tasks/primitive/lift_beaker_series.py`
