...
本指南描述如何利用 scihorizon_elab 的专家轨迹数据，通过 LeRobot 框架训练 Diffusion Policy (DP) 模型，并在 scihorizon_elab 中完成评测。
...
**项目根目录**：
- scihorizon_elab：`/ssd/qinmaokai/workspace/SciHorizon-ELAB`
- LeRobot：`/ssd/qinmaokai/workspace/lerobot`
...
| **数据格式** | 完全相同 | 完全相同 |
| **转换脚本** | `convert_to_lerobot_pi.py` | `convert_to_lerobot_pi.py` |
...
MUJOCO_GL=egl PYOPENGL_PLATFORM=egl \
    ...
    --n-sample 50 \
    --start-id 0 \
    --save-dir /ssd/qinmaokai/workspace/SciHorizon-ELAB/dataset/hdf5_trajectory \
    --record-video \
    --robot franka
```
...
python scripts/convert_to_lerobot_pi.py \
    ...
    --dataset-path /ssd/qinmaokai/workspace/SciHorizon-ELAB/dataset/hdf5_trajectory \
    --max-files 500 \
    --task-list <task_name>
```
...
```bash
cd /ssd/qinmaokai/workspace/lerobot

OUTPUT_DIR="/ssd/qinmaokai/workspace/SciHorizon-ELAB/models/diffusion_<task>_<N>ep_$(date +%Y%m%d_%H%M%S)"
...
| `crop_shape` | (84,84) | 默认裁剪，scihorizon_elab 设 None |
...
MUJOCO_GL=osmesa python scripts/evaluate_policy.py \
    ...
    --n-episode 10 \
    --policy dp \
    --model_ckpt /ssd/qinmaokai/workspace/SciHorizon-ELAB/models/diffusion_<task>_<N>ep_<date>/checkpoints/<step>/pretrained_model \
    --replanstep 4 \
...
SCIHORIZON_ELAB_ROOT="/ssd/qinmaokai/workspace/SciHorizon-ELAB"
LEROBOT_ROOT="/ssd/qinmaokai/workspace/lerobot"
HDF5_DIR="${SCIHORIZON_ELAB_ROOT}/dataset/hdf5_trajectory"
MODEL_DIR="${SCIHORIZON_ELAB_ROOT}/models/${MODEL_NAME}"
