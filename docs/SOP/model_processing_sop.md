...
你是 scihorizon_elab 项目的**资产处理专家**。你的职责是将已下载的 GLB 3D 模型转换为 scihorizon_elab 仿真框架可直接使用的格式（OBJ + MuJoCo XML），并在项目代码中完成注册，使其能在仿真任务中被正常加载、渲染和操作。

**完整链路**：下载的 GLB → 拆分（如多合一）→ 处理（OBJ/collision/XML）→ 注册（constant.py + normalizer）→ 可用

**项目根目录**：`/ssd/qinmaokai/workspace/SciHorizon-ELAB`（下文所有相对路径均基于此）
...
| 脚本 | 路径 | 用途 |
|------|------|------|
| **split_glb.py** | `scihorizon_elab/pipeline/tests/split_glb.py` | 将包含多个几何体的 GLB 文件拆分为独立 GLB |
| **process_local_glb.py** | `scihorizon_elab/pipeline/tests/process_local_glb.py` | GLB → OBJ 转换、collision mesh 生成、尺寸归一化、XML 生成 |
| **register_model.py** | `scihorizon_elab/pipeline/tests/register_model.py` | XML 注入（grasppoint/solution 等）+ 注册到 constant.py |
| **validate_asset.py** | `scihorizon_elab/pipeline/tests/validate_asset.py` | 验证生成的模型能否被 MuJoCo 正确加载 |
...
所有处理好的模型保存在：
```
scihorizon_elab/assets/review/<model_name>/<model_name>/
```

**标准输出结构**（以 `large_beaker` 为例）：
```
scihorizon_elab/assets/review/large_beaker/
  └── large_beaker/
...
| 文件 | 路径 | 作用 |
|------|------|------|
| **constant.py** | `scihorizon_elab/configs/constant.py` | 资产注册表 `name2class_xml`，框架通过此文件查找模型 |
| **normalizer.py** | `scihorizon_elab/pipeline/nodes/normalizer.py` | `STANDARD_ASSET_LIBRARY` 列表，LLM 分类时匹配模型名称 |
| **asset_cache.json** | `scripts/scihorizon_elab/assets/asset_cache.json` | Normalizer 缓存，已分类的模型会缓存 spec 映射 |
...
python scihorizon_elab/pipeline/tests/process_local_glb.py \
    --input_dir <GLB所在目录> \
    --keyword <模型关键词> \
    --output_dir scihorizon_elab/assets/review/<model_name>
```
...
```bash
python scihorizon_elab/pipeline/tests/register_model.py \
    --model_dir scihorizon_elab/assets/review/<model_name>/<model_name>/<model_name> \
    --class_name <实体类名> \
    --name <注册名>
```
...
编辑 `scihorizon_elab/pipeline/nodes/normalizer.py`，在 `STANDARD_ASSET_LIBRARY` 列表中添加新模型名：
...
如果 `scripts/scihorizon_elab/assets/asset_cache.json` 中已有该模型的旧记录（spec 指向错误的名称），需要修正：

```python
import json
with open('scripts/scihorizon_elab/assets/asset_cache.json', 'r') as f:
    cache = json.load(f)
...
with open('scripts/scihorizon_elab/assets/asset_cache.json', 'w') as f:
...
model = mujoco.MjModel.from_xml_path('scihorizon_elab/assets/review/<model_name>/<model_name>/<model_name>/<model_name>.xml')
print(f'OK: {model.nbody} bodies, {model.ngeom} geoms, {model.nsite} sites')
"

# 2. 验证 constant.py 注册
grep '<model_name>' scihorizon_elab/configs/constant.py
...
python scihorizon_elab/pipeline/tests/validate_asset.py --asset_dir scihorizon_elab/assets/review/<model_name>
```
...
glb_path = Path('scihorizon_elab/assets/review/chemistry_glassware.glb')
output_dir = Path('/ssd/qinmaokai/download')  # 临时输出目录
...
python scihorizon_elab/pipeline/tests/split_glb.py /path/to/combined.glb --output_dir /tmp/split

# 为每个模型创建临时目录
mkdir -p /tmp/<model_name>_input
cp /ssd/qinmaokai/download/<split_file>.glb /tmp/<model_name>_input/<model_name>.glb
...
`process_local_glb.py` 的 `--output_dir` 如果指定 `scihorizon_elab/assets/review/<name>`，实际 XML 会在：
```
scihorizon_elab/assets/review/<name>/<name>/<name>/<name>.xml
```

三层嵌套！`register_model.py` 的 `--model_dir` 必须指向最内层目录：
```bash
python scripts/register_model.py \
    --model_dir scihorizon_elab/assets/review/cylinder_small/cylinder_small/cylinder_small \
    ...
```
...
python scihorizon_elab/pipeline/tests/process_local_glb.py \
    ...
    --output_dir scihorizon_elab/assets/review/my_new_model \
    --rotate_axis x --rotate_degrees 90

# ===== Step 2: 注册 =====
python scihorizon_elab/pipeline/tests/register_model.py \
    --model_dir scihorizon_elab/assets/review/my_new_model/my_new_model \
...
import mujoco, os
os.environ['MUJOCO_GL'] = 'osmesa'
m = mujoco.MjModel.from_xml_path('scihorizon_elab/assets/review/my_new_model/my_new_model/my_new_model/my_new_model.xml')
print(f'OK: {m.nbody} bodies, {m.ngeom} geoms, {m.nsite} sites')
"
grep 'my_new_model' scihorizon_elab/configs/constant.py
