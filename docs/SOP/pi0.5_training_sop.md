...
你是 scihorizon_elab 项目的 **pi0.5 (π₀.₅) 模型训练与部署专家**。你的职责是将 scihorizon_elab 任务的专家轨迹（HDF5）经过格式转换、openpi 训练框架的 fine-tune，最终部署为可推理的策略模型，并验证其在仿真环境中的成功率。

**完整链路**：

```
scihorizon_elab 任务 (HDF5 轨迹)
  → scripts/convert_to_lerobot_pi.py (格式转换)
...
| 脚本 | 路径 | 用途 |
|------|------|------|
| `convert_to_lerobot_pi.py` | `scripts/convert_to_lerobot_pi.py` | HDF5 → LeRobot Dataset 格式转换（lerobot 0.1.0 API） |
...
| `evaluate_openpi.py` | `scripts/evaluate_openpi.py` | scihorizon_elab 客户端评测（WebSocket client + env loop + 可视化） |
...
| conda env | `vlabench_2` at `/ssd/qinmaokai/.conda/envs/vlabench_2` | scihorizon_elab client 端环境 |
...
snapshot_download(
    repo_id='scihorizon_elab/pi05-primitive-10task',
...
cp -r /tmp/extract/scihorizon_elab-openpi-pi05/. /ssd/qinmaokai/workspace/SciVLABench/third_party/openpi/
```
...
#### 3a. `scripts/convert_to_lerobot_pi.py` 修复

原脚本有 5 个问题需修：

```python
# 1. 删硬编码路径（line 2）
# 原：sys.path.insert(0, "/ssd/qinmaokai/workspace/lerobot")
...
TrainConfig(
    ...
    data=LeRobotVLABenchDataConfig(
        repo_id="<your_task>",                   # 必须与 convert_to_lerobot_pi.py --dataset-name 一致
...
**注意事项**：
- `repo_id` 必须等于 `convert_to_lerobot_pi.py --dataset-name`
...
def __call__(self, data: DataDict) -> DataDict:
    ...
    # 与 scihorizon_elab client 的 slash-separated keys (评测 client)：
    # 同时尝试两种分隔符匹配源 path。
...
export PYTHONPATH=/ssd/qinmaokai/workspace/SciVLABench/third_party/openpi/src:/ssd/qinmaokai/workspace/SciVLABench:${PYTHONPATH:-}
export MUJOCO_GL=egl
export SCIHORIZON_ELAB_ROOT=/ssd/qinmaokai/workspace/SciVLABench/scihorizon_elab
cd /ssd/qinmaokai/workspace/SciVLABench
...
`evaluate_openpi.py --tasks <name>` 要求 `<name>` 在 scihorizon_elab task registry 中。如果报 `KeyError: '<name>'`：

1. 检查 `scihorizon_elab/configs/__init__.py` 是否有 `"<name>_series": ["<name>"]`
...
python -c "from scihorizon_elab.utils.register import register; from scihorizon_elab.tasks import *; print(sorted(register._tasks.keys()))"
```
...
文档假设 conda 在 `/opt/miniconda3`，但实际可能是：
- `/opt/miniconda3`（系统装）
...
修改的文件：
- `scripts/convert_to_lerobot_pi.py` — 修硬编码路径、task 字段、save_episode、consolidate
