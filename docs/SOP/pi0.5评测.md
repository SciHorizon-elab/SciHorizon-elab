...
- **OpenPI 服务器环境**: `/ssd/qinmaokai/workspace/SciHorizon-ELAB/third_party/openpi/examples/vlabench/.venv`
- **scihorizon_elab 客户端环境**: `vlabench_2` conda 环境

### 2. 文件准备

- **Checkpoint**: 训练好的模型权重
- **任务文件**: 已注册的任务（`scihorizon_elab/tasks/autogen_tasks/primitive/*_series.py`）
- **Config 注册**: 任务已在 `scihorizon_elab/configs/__init__.py` 的 `name2config` 中注册
...
# 启动服务器（后台运行）
nohup /ssd/qinmaokai/workspace/SciHorizon-ELAB/third_party/openpi/examples/vlabench/.venv/bin/python \
...
- `<CHECKPOINT_PATH>`: checkpoint 路径（如 `/ssd/qinmaokai/workspace/SciHorizon-ELAB/third_party/openpi/checkpoints/pi0.5_ft_place_flask/pi0.5_ft_place_flask_100K/60000`）
...
nohup /ssd/qinmaokai/workspace/SciHorizon-ELAB/third_party/openpi/examples/vlabench/.venv/bin/python \
    ...
    --policy.dir=/ssd/qinmaokai/workspace/SciHorizon-ELAB/third_party/openpi/checkpoints/pi0.5_ft_place_flask/pi0.5_ft_place_flask_100K/60000 \
    > /tmp/pi05_server.log 2>&1 &
...
**解决**:
1. 确认任务在 `scihorizon_elab/configs/__init__.py` 的 `name2config` 中有映射
2. 确认任务文件存在于 `scihorizon_elab/tasks/autogen_tasks/primitive/`
...
**可能原因**:
1. **数据问题**: 使用了修复前的 `convert_to_lerobot_pi.py`（gripper 阈值 0.03 导致 lock 宽度误判为张开）
...
2. 观看视频确认具体失败位置
3. 检查训练数据是否使用了正确的 `convert_to_lerobot_pi.py`
...
# 启动 server 模板
cd /ssd/qinmaokai/workspace/SciHorizon-ELAB/third_party/openpi
export PYTHONPATH=/ssd/qinmaokai/workspace/SciHorizon-ELAB/third_party/openpi/src:/ssd/qinmaokai/workspace/SciVLABENCH:$PYTHONPATH
export HF_ENDPOINT=https://hf-mirror.com
export MUJOCO_GL=egl
export CUDA_VISIBLE_DEVICES=7
...
| `scihorizon_elab/tasks/autogen_tasks/primitive/*_series.py` | 任务定义 |
| `scihorizon_elab/configs/__init__.py` | 任务注册 |

---

## 维护者

scihorizon_elab Team
