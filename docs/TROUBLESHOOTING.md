# scihorizon_elab 常见错误与解决方案

本文档记录 scihorizon_elab 项目开发过程中遇到的常见错误、根本原因及解决方案，便于团队成员快速定位和解决类似问题。
...
6. [机械臂倾倒锥形瓶时瓶子从夹爪中滑出](#6-机械臂倾倒锥形瓶时瓶子从夹爪中滑出)
...
scihorizon_elab 中场景配置了候选位置偏移（`scihorizon_elab/simulation/configs/scene_config.json`）：
...
关键代码（`scihorizon_elab/benchmark/tasks/dm_task.py`）：
...
**文件**: `scihorizon_elab/configs/task_config.json`
...
`scihorizon_elab/tasks/components/scene.py`:
...
`scihorizon_elab/envs/__init__.py`:
...
```python
import json
with open('scihorizon_elab/configs/task_config.json') as f:
    config = json.load(f)
    task_name = 'your_task'
    ...
    # 检查场景偏移
    with open('scihorizon_elab/simulation/configs/scene_config.json') as sf:
...
- `scihorizon_elab/configs/task_config.json` - 任务配置
- `scihorizon_elab/simulation/configs/scene_config.json` - 场景配置
- `scihorizon_elab/configs/robot_config.json` - 机器人默认配置
- `scihorizon_elab/tasks/components/scene.py` - 场景加载逻辑
- `scihorizon_elab/envs/__init__.py` - 环境初始化
- `scihorizon_elab/benchmark/tasks/dm_task.py` - 任务基类
...
**文件**: `scihorizon_elab/assets/obj/meshes/lab_equipment/petri_dish/petri_dish_0/petri_dish.obj`

1. **备份原始文件**
   ```bash
   cd scihorizon_elab/assets/obj/meshes/lab_equipment/petri_dish/petri_dish_0
...
- `scihorizon_elab/assets/obj/meshes/lab_equipment/petri_dish/petri_dish_0/petri_dish.obj` - 视觉网格模型
- `scihorizon_elab/assets/obj/meshes/lab_equipment/petri_dish/petri_dish_0/petri_dish.xml` - MuJoCo模型配置
- `scihorizon_elab/tasks/components/entity.py` - 实体基类（包含 `is_grasped()` 方法）
...
**日期**: 2026-05-25
**影响组件**: `scihorizon_elab/tasks/condition.py` — `LiftCondition`
...
文件：`scihorizon_elab/tasks/condition.py`
...
```python
from scihorizon_elab.robots import *
from scihorizon_elab.tasks import *
from scihorizon_elab.envs import load_env
...
- `scihorizon_elab/tasks/condition.py` — 条件定义
- `scihorizon_elab/envs/dm_env.py` — `reset()` 和 `step()` 中的终止检查
- `scihorizon_elab/benchmark/tasks/dm_task.py` — `should_terminate_episode()` 调用 `conditions.is_met()`
...
- 但 scihorizon_elab 的 pipeline 中，一个技能结束后还有后续动作（open_gripper、lift），不能立刻 reset
...
**文件**: `scihorizon_elab/envs/dm_env.py`
...
**文件**: `scihorizon_elab/pipeline/nodes/simulation.py`
...
- `scihorizon_elab/envs/dm_env.py` — `step()` 方法，`_skill_execution_mode` 开关
- `scihorizon_elab/pipeline/nodes/simulation.py` — 技能循环控制开关
- `scihorizon_elab/utils/skill_lib.py` — `place()`, `drop()`, `step_trajectory()`
- `scihorizon_elab/tasks/condition.py` — `OnCondition.is_met()`
- `scihorizon_elab/benchmark/tasks/dm_task.py` — `should_terminate_episode()`
...
**影响组件**: `scihorizon_elab/tasks/condition.py` — `CapOpenCondition`，`scihorizon_elab/tasks/components/specific_entities/interactive_containers.py` — `ContainerWithCap.is_cap_open`
...
- `scihorizon_elab/tasks/condition.py` — `CapOpenCondition` 修复
- `scihorizon_elab/tasks/components/specific_entities/interactive_containers.py` — `ContainerWithCap.is_cap_open` 修复
- `scihorizon_elab/tasks/autogen_tasks/unscrew_pill_bottle_series.py` — `build_from_config` 中的 attach 逻辑
- `scihorizon_elab/assets/review/pill_bottle/pill_bottle/pill_bottle/pill_bottle.xml` — pill_bottle XML（hinge + slide joint 定义）
...
**日期**: 2026-08-30
**影响任务/组件**: 任何使用 `SkillLib.pour_to_entity` 倾倒容器（尤其是 flask、tube 等）的任务
**严重程度**: 🔴 高（倾倒任务完全失败，瓶子从夹爪中脱落）

### 问题表现

1. **倾倒过程中瓶子沿指面滑出**
   - 锥形瓶被 Franka 夹爪抓在瓶颈处，机械臂绕末端关节旋转倾倒
   - 旋转产生的离心力 + 瓶子重力沿指面切向分量，使瓶子沿指面切向滑动
   - 视频可观察到瓶子从夹爪中"出溜"直至完全脱出

2. **异常现象：减慢倾倒速度反而更糟**
   - 把 `tilt_velocity` 减半 → **滑得更厉害**
   - 把 `tilt_velocity` 加快 → 反而能在瓶子滑出前完成倾倒
   - 这与"经典摩擦力不足"的直觉相反（μ·N 不够的话快慢都应该掉）

3. **增大 μ 无效**
   - 把 `friction` 从 `1 1 0.1` 提到 `4 4 0.1`
   - 加上 `condim="6"` 启用扭转/滚动摩擦
   - 滑出症状无明显改善

### 根本原因：MuJoCo 摩擦约束的"蠕动"（creep），不是经典摩擦力不足

#### 1. 软约束的本质

MuJoCo 的接触约束是**软约束**（基于优化的阻抗 + 正则化），不是硬约束。这意味着即使切向力远低于 μN，每个时间步仍允许微小切向滑移 Δs。这是 MuJoCo 求解器在计算成本和约束严格度之间的折衷。
...
- 接触时间越长 → 累积滑移越大
- 减半 `tilt_velocity` = 倾倒时间翻倍 = 滑移累积翻倍 → **更滑**
- 加快 `tilt_velocity` = 时间减半 = 累积减半 → **看起来好了（治标不治本）**

#### 3. 实验验证（在你的 mujoco 3.2.2 环境中实测）

```
斜面蠕动实验: 立方体放在 15° 斜面上（μ=1, 切向重力分量 < μN，理论上应纹丝不动）
              模拟 2 秒后测量位移

配置                          | 2 秒滑移
------------------------------|----------
pyramidal (原配置)             | 0.278 mm ← 蠕动存在
elliptic                       | 0.132 mm
elliptic + noslip_iterations=5 | 0.000 mm ← 蠕动消除
elliptic + noslip_iterations=10| 0.000 mm
```

锥形瓶抓在瓶颈上是**长力臂杠杆**，指面上 0.3mm 的蠕动在瓶子顶部会被放大数倍，表现为肉眼可见的"出溜"。

#### 4. 为什么现有参数都救不了

查运行时实际生效的物理配置：仿真根 XML 是 [base/default.xml](../scihorizon_elab/simulation/assets/base/default.xml)（由 [dm_task.py:40](../scihorizon_elab/benchmark/tasks/dm_task.py#L40) 加载），场景文件里的 `<option>` 整块被注释掉了。当前只设了 `impratio=10`（已有），缺两个关键开关：

| 参数 | 默认值 | 状态 | 影响 |
|---|---|---|---|
| `noslip_iterations` | **0（禁用）** | ❌ | 没有启用消除蠕动的后处理求解器 |
| `cone` | **pyramidal** | ❌ | 即使开了 `noslip_iterations`，它**只在 `elliptic` 锥下生效**（[官方文档](https://mujoco.readthedocs.io/en/stable/computation/index.html)） |

有意思的是，场景文件里有一行被注释的 `<option impratio="1" noslip_iterations="3"/>`——之前有人试过开 noslip，但 (1) 在注释块里没生效；(2) 即便生效也缺 `cone="elliptic"`，noslip 根本不会工作。

### 解决方案

**一行改动，启用 noslip + elliptic 锥**（根因修复，直接消灭蠕动）：

**文件**: `scihorizon_elab/simulation/assets/base/default.xml`
...
1. **物理层（必需）**：上面这行改动
2. **资产层（强化）**：给容器（flask、tube 等）加 **Anti-slip 抓夹板**，提升局部摩擦并给手指 pad 提供机械卡位

#### Anti-slip 抓夹板的规范

参考 [flask.xml](../scihorizon_elab/simulation/assets/obj/flask/flask/flask.xml) 和 [tube.xml](../scihorizon_elab/simulation/assets/obj/tube/tube.xml) 已有的实现：

```xml
<!-- 4 块视觉板（红色调试用，不参与碰撞） -->
<geom name="grip_plate_x_v_1" type="box" size="0.0105 0.005 0.027" pos="0.020 0.020 0.173"
      quat="0.9238795 0 0 0.3826834"
      contype="0" conaffinity="0" group="2" rgba="1 0 0 0.8"/>
<!-- ... 3 块类似 ... -->

<!-- 4 块碰撞板（只和手指 pad 碰撞，contype=2 conaffinity=2） -->
<geom name="grip_plate_x" type="box" size="0.0105 0.005 0.027" pos="0.020 0.020 0.173"
      quat="0.9238795 0 0 0.3826834"
      contype="2" conaffinity="2" group="3"
      condim="6" friction="4 4 0.1" density="50" solref="0.001 1"/>
<!-- ... 3 块类似 ... -->
```

**关键参数**：
- `condim="6"` —— 启用扭转+滚动摩擦（MuJoCo 接触维度）
- `friction="4 4 0.1"` —— μ=4（滑动）, μ=4（扭转）, μ=0.1（滚动），μ=4 是仿真抓取常用值
- `contype="2" conaffinity="2"` —— 只与手指 pad（contype=2, conaffinity=3）碰撞，不与桌面、其他容器冲突
- 4 块板子旋转 ±45° 交叉形成 X 形，给手指 pad 提供"凹槽"卡位

**不同容器的几何适配要点**：

| 容器 | 旋转轴 | 长轴方向 | 板子 size | pos | 板-壁关系 |
|---|---|---|---|---|---|
| **flask**（瓶颈圆柱） | Z 轴 ±45° | 世界 Z（瓶身轴） | 0.0105×0.005×0.027 | (0.020, 0.020, 0.173) | 贴瓶颈外壁 r=0.022 |
| **tube**（试管） | **Y 轴 ±45°** | body local Y（管长方向） | 0.012×0.024×0.003 | (0.028, 0.14, 0) | 贴管壁 r=0.024 |
| **beaker_small**（烧杯） | Z 轴 ±45° | 世界 Z | 0.0105×0.005×0.0446 | (0.0286, 0.0286, 0.0446) | 贴烧杯壁 |
| **aspirin_pill_bottle**（瓶盖） | Z 轴 ±45° | 世界 Z | 0.0411×0.003×0.0103 | (0, 0, 0.0103) | 贴瓶盖外壁 |

**注意 tube 的特殊性**：tube body 用了 `euler="1.57 0 0"`（绕 X 轴 90°）使试管竖起，所以管长方向在 body 局部是 **Y 轴**而不是 Z 轴。quat 应该绕 **Y 轴**旋转（不是 Z 轴），才能让板子的"长"沿管长方向。语法上：`quat="0.9238795 0 0.3826834 0"`（w, x, **y**, z），y 分量控制绕 Y 轴的旋转角。
...
def sim(cone, noslip, nsteps=2000):
    xml = f"""
<mujoco>
  <option timestep="0.001" impratio="10" cone="{cone}" noslip_iterations="{noslip}" integrator="implicitfast"/>
  <worldbody>
    <geom name="ramp" type="plane" size="10 10 0.1"/>
    <body name="box" pos="0 0 0.05">
      <freejoint/>
      <geom name="boxgeom" type="box" size="0.05 0.05 0.05" friction="1 1 0.01" condim="4"/>
    </body>
  </worldbody>
</mujoco>"""
    ...
    d = mujoco.MjData(m)
    theta = np.deg2rad(15)
    m.opt.gravity = np.array([9.81*np.sin(theta), 0, -9.81*np.cos(theta)])
    for _ in range(500):  # 稳定落下
        mujoco.mj_step(m, d)
    x0 = d.body('box').xpos[0].copy()
    for _ in range(nsteps):
        mujoco.mj_step(m, d)
    return d.body('box').xpos[0] - x0

for cone, noslip in [("pyramidal", 0), ("elliptic", 0), ("elliptic", 5), ("elliptic", 10)]:
    drift = sim(cone, noslip)
    print(f"{cone} noslip={noslip}: 2s 滑移 = {drift*1000:.3f} mm")
```

预期输出：
```
pyramidal 0: 0.278 mm    ← 蠕动
elliptic 0: 0.132 mm
elliptic 5: 0.000 mm    ← 蠕动消除
elliptic 10: 0.000 mm
```
...
```python
import mujoco
m = mujoco.MjModel.from_xml_path('scihorizon_elab/simulation/assets/base/default.xml')
...
应该看到 `cone=1, noslip_iterations=5`。

#### 3. 倾倒任务验证

```python
from scihorizon_elab.envs import load_env
env = load_env('pick_flask_pour_object_series')
obs = env.reset()
# 记录瓶子初始位置
init_xpos = env.task.entities['flask_0'].get_xpos(env.physics).copy()
# 执行倾倒
...  # 调用 SkillLib.pour_to_entity
# 检查瓶子位置
final_xpos = env.task.entities['flask_0'].get_xpos(env.physics).copy()
drift = np.linalg.norm(final_xpos - init_xpos)
print(f'瓶子位移: {drift*1000:.1f}mm')  # 修复后应该 < 5mm
```
...
| 现象 | 失败配置 | 修复配置 |
|---|---|---|
| 锥形瓶从夹爪滑出 | 锥形瓶在倾倒 60° 时脱出 | 锥形瓶保持夹紧直到倾倒完成 |
| 试管被甩出 | 试管在倾倒中途飞离夹爪 | 试管平稳跟随夹爪倾斜 |
| 减慢速度更糟 | tilt_velocity 减半 → 滑出更早 | 速度变化不影响 |

### 预防措施

#### ✅ DO — 处理任何"抓取后运动"任务

1. **抓夹板必备三件套**（加到所有可抓取容器的 XML）：
   - `condim="6"`（接触维度，含扭转+滚动）
   - `friction="4 4 0.1"`（μ=4 抓取推荐值）
   - `contype="2" conaffinity="2"`（只与手指 pad 碰撞）

2. **几何适配**：参考上表，对不同容器调整 pos、size 和 quat 旋转轴

3. **仿真精度优先**：核心物理参数用 `cone="elliptic" noslip_iterations="5"`，避免任何需要靠速度/时间博弈的滑移累积

#### ❌ DON'T

1. **不要只调速度/时间**：蠕动是约束层面的问题，调速度只改变累积时间，治标不治本
2. **不要只调 μ**：μ 决定的是"是否进入滑动状态"，蠕动是"未滑动时也有微小位移"，两者机制独立
3. **不要忘了 `cone="elliptic"`**：只开 `noslip_iterations` 而用默认 `pyramidal` 锥，noslip 完全无效（[MuJoCo 文档](https://mujoco.readthedocs.io/en/stable/computation/index.html)）
4. **不要给 tube 用绕 Z 轴的 quat**：tube body 有 `euler="1.57 0 0"`，管长在 body 局部 Y 轴，必须绕 Y 旋转
5. **不要把抓夹板的 contype 设成 1**：会和场景其他物体碰撞（桌面、其他容器），把瓶子"焊死"

### 相关文件

- `scihorizon_elab/simulation/assets/base/default.xml` — 物理 option（**关键改动点**）
- `scihorizon_elab/simulation/assets/obj/flask/flask/flask.xml` — flask 抓夹板实现
- `scihorizon_elab/simulation/assets/obj/tube/tube.xml` — tube 抓夹板实现（绕 Y 轴特例）
- `scihorizon_elab/simulation/assets/obj/beaker_small/small_beaker/small_beaker.xml` — beaker 抓夹板
- `scihorizon_elab/simulation/assets/obj/aspirin_pill_bottle/aspirin_pill_bottle/aspirin_pill_bottle/aspirin_pill_bottle.xml` — aspirin_pill_bottle 抓夹板
- `scihorizon_elab/utils/skill_lib.py:980` — `pour_to_entity` 函数定义
- `scihorizon_elab/simulation/assets/robots/franka_emika_panda/panda.xml:240,251` — 手指 pad（已有 condim=4）

### 关键经验

> **原则**：MuJoCo 摩擦约束是软约束，蠕动是默认行为。任何"长时间接触 + 切向力"的场景（抓取倾倒、堆叠、长距离滑行），必须显式启用 `noslip_iterations` + `elliptic` 锥，否则就会有累积滑移。

> **诊断技巧**：如果"减慢动作反而更糟"，就是蠕动而不是经典摩擦力问题——快慢都在滑，慢的累积时间更长。遇到这种症状直接去查 noslip/elliptic 配置。

> **修复优先级**：`cone + noslip`（根因）> `condim`（接触维度）> `friction`（μ 值）> 抓夹板几何（强化）。前三项是物理引擎层面的，必须先做对；抓夹板是资产层面的锦上添花。

---

**最后更新**: 2026-08-30
**维护者**: scihorizon_elab Team
