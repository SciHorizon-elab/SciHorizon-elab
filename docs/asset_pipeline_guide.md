...
本 Pipeline 实现了从 Objaverse 开源 3D 模型库自动下载模型、转换为 MuJoCo MJCF 格式、后处理修复、物理验证的完整流程。目标是让下载的模型能够直接加载到 scihorizon_elab 仿真场景中使用。

### 核心问题

从 Objaverse 下载的原始模型无法直接在 scihorizon_elab 中使用，存在以下问题：
...
| 文件 | 用途 | 类型 |
|------|------|------|
| `scihorizon_elab/pipeline/tools/get_assets.py` | 主 Pipeline：搜索、下载、转换、后处理、验证 | 修改 |
| `fix_obj2mjcf_xml.py` | OBJ 居中、尺寸归一化、XML 物理属性修复 | 修改 |
| `scihorizon_elab/pipeline/tests/validate_asset.py` | MuJoCo 加载验证 + 视频录制 | 新建 |
...
从 Objaverse（~80万个3D模型）搜索并下载模型，经过完整处理后输出可用于 scihorizon_elab 的 MJCF 模型。
...
```bash
cd /ssd/qinmaokai/workspace/SciHorizon-ELAB/scripts
...
# 验证单个模型（仅物理检查）
python scihorizon_elab/pipeline/tests/validate_asset.py path/to/model.xml

# 验证 + 录制视频（推荐）
python scihorizon_elab/pipeline/tests/validate_asset.py path/to/model.xml --video

# 验证 + 截图
python scihorizon_elab/pipeline/tests/validate_asset.py path/to/model.xml --render

# 批量验证 + 录制视频
python scihorizon_elab/pipeline/tests/validate_asset.py --dir scripts/assets/review/beaker/ --video

# 自定义视频参数
python scihorizon_elab/pipeline/tests/validate_asset.py path/to/model.xml --video --video-duration 5.0 --video-fps 60
```
