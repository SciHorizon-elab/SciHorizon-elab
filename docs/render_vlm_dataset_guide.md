...
`render_vlm_dataset.py` 是 scihorizon_elab 的核心渲染工具,用于为 VLM 评测任务生成高质量的多视角图像和实例分割掩码。该工具集成了自动化场景验证系统,确保渲染前模型放置正确。
...
# 渲染单个任务 (自动检测样本数量)
python scihorizon_elab/pipeline/tools/render_vlm_dataset.py --task insert_tube_centrifuge --dimension "M&T"

# 渲染指定数量的样本
python scihorizon_elab/pipeline/tools/render_vlm_dataset.py --task pour_tube --dimension "M&T" --num-examples 5

# 覆盖已存在的图像
python scihorizon_elab/pipeline/tools/render_vlm_dataset.py --task lift_object --dimension "M&T" --overwrite
```
...
```bash
# 1. 创建任务配置
python scihorizon_elab/pipeline/tools/create_vlm_task.py --task insert_tube_centrifuge --num-examples 3

# 2. 渲染图像 + 自动验证
python scihorizon_elab/pipeline/tools/render_vlm_dataset.py --task insert_tube_centrifuge --dimension "M&T" --overwrite
...
#### `--task` (必需)
- 指定要渲染的任务名称
- 任务必须在 `scihorizon_elab/pipeline/tools/create_vlm_task.py` 的 `BUILTIN_TEMPLATES` 中定义
- 对应的 `env_config.json` 文件必须已存在于数据集目录

**可用任务列表:**
```bash
# 查看所有可用任务
python scihorizon_elab/pipeline/tools/create_vlm_task.py --list-templates
```
...
#### `--cameras`
- 指定使用哪些相机角度
- scihorizon_elab 默认有 4 个固定相机 (ID: 0, 1, 2, 3)
...
```bash
# 场景: 离心机试管插入任务, 3 个样本
python scihorizon_elab/pipeline/tools/render_vlm_dataset.py \
...
```bash
# 1. 首次渲染 (发现问题)
python scihorizon_elab/pipeline/tools/render_vlm_dataset.py --task my_task --dimension "M&T"
...
# 2. 修改模型 XML
vim scihorizon_elab/assets/obj/meshes/lab_equipment/centrifuge/centrifuge.xml
# 添加: <body name="centrifuge" euler="1.57 0 0" pos="0 0 0.12">

# 3. 重新渲染验证
python scihorizon_elab/pipeline/tools/render_vlm_dataset.py --task my_task --dimension "M&T" --overwrite
...
```bash
source /opt/miniconda3/etc/profile.d/conda.sh && conda activate vlabench_2
python scihorizon_elab/pipeline/tools/render_vlm_dataset.py --task your_task --dimension "M&T"
```
...
# 2. 减少相机数量
python scihorizon_elab/pipeline/tools/render_vlm_dataset.py --task your_task --cameras 0 2
...
os.environ['SCIHORIZON_ELAB_ROOT'] = project_root  # 项目根目录
```

### 相机配置

scihorizon_elab 默认使用 4 个固定相机:
...
```python
# 不推荐: 逐个样本手动运行
python scihorizon_elab/pipeline/tools/render_vlm_dataset.py --task my_task --num-examples 1
python scihorizon_elab/pipeline/tools/render_vlm_dataset.py --task my_task --num-examples 2
...

# 推荐: 一次性批量渲染
python scihorizon_elab/pipeline/tools/render_vlm_dataset.py --task my_task --num-examples 100
```
...
parallel -j 4 python scihorizon_elab/pipeline/tools/render_vlm_dataset.py --task {} --dimension "M&T" ::: \
...
```xml
<!-- scihorizon_elab/assets/scenes/laboratory_0/laboratory_0.xml -->
<worldbody>
...
使用新相机:
```bash
python scihorizon_elab/pipeline/tools/render_vlm_dataset.py --task my_task --cameras 0 1 2 3 4
```
...
- **scihorizon_elab 官方文档**: [README.md](../README.md)
...
- ✨ 支持高度、朝向、碰撞、可见性检查
- 🐛 修复 body 名称映射问题 (scihorizon_elab 分层结构)
- 📝 完善文档和使用示例
...
- **文档**: `/ssd/qinmaokai/workspace/SciHorizon-ELAB/docs/`
- **代码**: `/ssd/qinmaokai/workspace/SciHorizon-ELAB/scihorizon_elab/pipeline/tools/render_vlm_dataset.py`
