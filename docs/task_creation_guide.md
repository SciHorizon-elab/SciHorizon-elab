...
3. 在 benchmark/configs/__init__.py 的 name2config 中注册
4. 在 benchmark/configs/task_config.json 中添加配置
...
**路径**：`scihorizon_elab/benchmark/tasks/autogen_tasks/primitive/<task_name>_series.py`
...
from scihorizon_elab.benchmark.tasks.autogen_tasks.primitive.base import PrimitiveTask
...
**路径**：`scihorizon_elab/benchmark/tasks/autogen_tasks/primitive/__init__.py`

在文件末尾追加一行：

```python
from scihorizon_elab.benchmark.tasks.autogen_tasks.primitive.my_task_series import *
```
...
## 第三步：在 benchmark/configs/__init__.py 中注册

**路径**：`scihorizon_elab/benchmark/configs/__init__.py`
...
**路径**：`scihorizon_elab/benchmark/configs/task_config.json`
...
使用 `scihorizon_elab/pipeline/tools/trajectory_generation.py`：

```bash
# 基本用法
MUJOCO_GL=egl python3 scihorizon_elab/pipeline/tools/trajectory_generation.py \
    --task-name my_task \
    --n-sample 1 \
    --debug \
...
| `benchmark/configs/__init__.py` | 追加 name2config 条目 |
| `benchmark/configs/task_config.json` | 追加配置 |
...
MUJOCO_GL=egl python3 scihorizon_elab/pipeline/tools/trajectory_generation.py \
    ...
    --n-sample 1 \
    --debug \
    --save-dir /ssd/qinmaokai/workspace/SciHorizon-ELAB/test_output
```
...
- `benchmark/configs/__init__.py` 的 `name2config` 中是否包含该任务
