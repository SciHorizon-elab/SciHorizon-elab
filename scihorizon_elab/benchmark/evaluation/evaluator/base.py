...
from scihorizon_elab.utils.utils import euler_to_quaternion, quaternion_to_euler, quaternion_to_rotvec, rotvec_to_quaternion, find_key_by_value
    ...
    def evaluate_single_episode(self, agent, task_name, episode_id, episode_config, seed=42, max_episode_length=200, **kwargs):
        ...
        while i < max_episode_length:
            ...
            if last_action is None:
                if len(ee_state) == 8:  # [pos3, quat4, gripper1]（未走 env rotvec 路径的兼容分支）
                    last_action = np.concatenate([ee_state[:3], quaternion_to_rotvec(ee_state[3:7])])
                else:  # 7 维: [pos3, rotvec3, gripper1]
                    last_action = np.concatenate([ee_state[:3], ee_state[3:6]])
            ...
            if agent.control_mode == "ee":
                pos, rotvec, gripper_state = agent.predict(observation, **kwargs)
                last_action = np.concatenate([pos, rotvec])
