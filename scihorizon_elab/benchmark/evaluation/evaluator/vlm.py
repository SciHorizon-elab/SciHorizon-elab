...
import traceback
import torch
from scihorizon_elab.benchmark.evaluation.utils import *
...
class VLMEvaluator(Evaluator):
    def __init__(self, 
        ...
        with open(os.path.join(os.environ["SCIHORIZON_ELAB_ROOT"], f"configs/prompt/eval_vlm_{language}.txt"), 'r', encoding='utf-8') as file:
            self.pre_prompt = file.read()
        seq_independent_task_config_path = os.path.join(os.environ["SCIHORIZON_ELAB_ROOT"], "benchmark/configs/evaluation/seq_independent_task.json")
        with open(seq_independent_task_config_path, 'r') as f:
            self.seq_independent_task = json.load(f)
        dim2task_config_path = os.path.join(os.environ["SCIHORIZON_ELAB_ROOT"], "benchmark/configs/evaluation/dim2task.json")
