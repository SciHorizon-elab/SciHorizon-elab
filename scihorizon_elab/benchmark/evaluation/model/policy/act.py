...
class ACTPolicy(Policy):
    """
    ACT policy wrapper for scihorizon_elab.
    ...
    def process_observation(self, obs):
        """
        Convert scihorizon_elab observation dict to ACT input batch.

        scihorizon_elab format:
        ...
        # 当前(256×256): scihorizon_elab obs 是 480×480,checkpoint 训练时是 256×256,需 resize
    ...
    def predict(self, obs, unnorm_key=None, **kwargs):
        ...
        - Convert gripper signal to scihorizon_elab 2D format
        """
        # Reset check
        if self.timestep == 0:
            self.reset()
