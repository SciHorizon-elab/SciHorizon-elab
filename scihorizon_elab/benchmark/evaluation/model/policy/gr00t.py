...
class Gr00tPolicy:
    ...
    def predict(self, observation, **kwargs):
        if self.timestep%4 == 0:
            ...
            if len(state) == 8:  # [pos3, quat4, gripper1]（原始 env 格式）
                pos, quat, gripper_state = state[:3], state[3:7], state[-1]
                ee_rotvec = quaternion_to_rotvec(quat)
            else:  # 7 维 [pos3, rotvec3, gripper1]（env 端 unwrap 后格式）
            ...
            obs = {
                    ...
                    "state.end_effector_rotation_relative": np.expand_dims(ee_rotvec.squeeze(), axis=0),
        ...
        target_rotvec = self.action_eer.popleft()
