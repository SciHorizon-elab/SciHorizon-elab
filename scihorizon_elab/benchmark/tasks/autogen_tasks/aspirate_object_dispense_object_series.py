...
@register.add_config_manager("aspirate_object_dispense_object")
class AspirateObjectDispenseObjectConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        ...
        obj_config = dict(
            ...
            position=[random.uniform(-0.1520, -0.0150), random.uniform(0.1050, 0.2420), 0.8],
        )
        obj_config["class"] = "ChemistryContainer"
        obj_config["randomness"] = dict(pos=[0.0200, 0.0200, 0], quat=[0, 0, 0.05])
        self.config["task"]["components"].append(obj_config)
    ...
    def get_instruction(self, target_entity, init_container, **kwargs):
        self.config["task"]["instructions"] = ["Use the <pipette_0> to aspirate the <CuSO4 solution_0> from the <large_beaker_0>."]
...
@register.add_task("aspirate_object_dispense_object")
class AspirateObjectDispenseObjectTask(PrimitiveTask):
    def __init__(self, task_name, robot, **kwargs):
        super().__init__(task_name, robot=robot, **kwargs)

    def get_expert_skill_sequence(self, physics):
        skill_sequence = [
            partial(SkillLib.pick, target_entity_name="pipette_0", prior_eulers=[[-3.141592653589793, 0, 0]]),
