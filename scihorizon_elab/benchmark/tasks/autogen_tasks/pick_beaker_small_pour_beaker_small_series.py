...
@register.add_config_manager("pick_beaker_small_pour_beaker_small")
class PickBeakerSmallPourBeakerSmallConfigManager(BenchTaskConfigManager):
...
@register.add_task("pick_beaker_small_pour_beaker_small")
class PickBeakerSmallPourBeakerSmallTask(PrimitiveTask):
    def __init__(self, task_name, robot, **kwargs):
        super().__init__(task_name, robot=robot, **kwargs)

    def get_expert_skill_sequence(self, physics):
        skill_sequence = [
            partial(SkillLib.gently_pick, target_entity_name="small_beaker_0", prior_eulers=[[-3.141592653589793, 0, 0]]),
