...
@register.add_config_manager("pick_conical_flask_small")
class PickConicalFlaskSmallConfigManager(BenchTaskConfigManager):
    ...
    def get_instruction(self, target_entity, **kwargs):
        self.config["task"]["instructions"] = ["pick <conical_flask_small_0> which contains <CuSO4_0>"]
...
@register.add_task("pick_conical_flask_small")
class PickConicalFlaskSmallTask(PrimitiveTask):
    def __init__(self, task_name, robot, **kwargs):
        super().__init__(task_name, robot=robot, **kwargs)

    def get_expert_skill_sequence(self, physics):
        skill_sequence = [
            partial(SkillLib.pick, target_entity_name="conical_flask_small_0"),
        ]
