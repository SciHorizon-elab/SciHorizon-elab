...
@register.add_config_manager("pick_cylinder_mid_pour_object")
class PickCylinderMidPourObjectConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        obj_config = dict(
            ...
            position=[random.uniform(-0.3, -0.05), random.uniform(-0.15, 0.25), 0.8],
        ...
        obj_config = dict(
            ...
            position=[random.uniform(0.05, 0.3), random.uniform(-0.05, 0.20), 0.8],
        )
        obj_config["class"] = "ChemistryBeaker"
        obj_config["randomness"] = dict(pos=[0.02, 0.02, 0], quat=[0, 0, 0.05])
        self.config["task"]["components"].append(obj_config)
    ...
    def get_instruction(self, target_entity, **kwargs):
        self.config["task"]["instructions"] = ["pick the <cylinder_mid_0> which contains <CuSO4 solution_0>."]
...
@register.add_task("pick_cylinder_mid_pour_object")
class PickCylinderMidPourObjectTask(PrimitiveTask):
    def __init__(self, task_name, robot, **kwargs):
        super().__init__(task_name, robot=robot, **kwargs)

    def get_expert_skill_sequence(self, physics):
        skill_sequence = [
            partial(SkillLib.pick, target_entity_name="cylinder_mid_0", prior_eulers=[[-3.14159, 0, 0]]),
