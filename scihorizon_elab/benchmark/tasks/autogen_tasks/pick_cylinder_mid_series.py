...
@register.add_config_manager("pick_cylinder_mid")
class PickCylinderMidConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        obj_config = dict(
            ...
            position=[random.uniform(-0.0860, -0.0150), random.uniform(-0.0970, -0.0290), 0.8],
        )
        ...
        obj_config = dict(
            ...
            position=[random.uniform(-0.2920, -0.1550), random.uniform(0.0810, 0.2180), 0.8],
        )
        obj_config["class"] = "ChemistryContainer"
        obj_config["randomness"] = dict(pos=[0.0200, 0.0200, 0], quat=[0, 0, 0.05])
        self.config["task"]["components"].append(obj_config)
        ...
        obj_config = dict(
            ...
            position=[random.uniform(-0.2970, -0.1960), random.uniform(-0.1300, -0.0290), 0.8],
        )
    ...
    def get_instruction(self, target_entity, init_container, **kwargs):
        self.config["task"]["instructions"] = ["Pick the lift the <cylinder_mid_0>."]
...
@register.add_task("pick_cylinder_mid")
class PickCylinderMidTask(PrimitiveTask):
