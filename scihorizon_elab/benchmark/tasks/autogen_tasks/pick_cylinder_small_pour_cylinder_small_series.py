...
@register.add_config_manager("pick_cylinder_small_pour_cylinder_small")
class PickCylinderSmallPourCylinderSmallConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        obj_config = dict(
            ...
            position=[0.35, random.uniform(-0.15, -0.05), 0.8],
...
@register.add_task("pick_cylinder_small_pour_cylinder_small")
class PickCylinderSmallPourCylinderSmallTask(PrimitiveTask):
    def __init__(self, task_name, robot, **kwargs):
        super().__init__(task_name, robot=robot, **kwargs)

    def get_expert_skill_sequence(self, physics):
        skill_sequence = [
            partial(SkillLib.pick, target_entity_name="cylinder_small_0", prior_eulers=[[-3.141592653589793, 0, 0]], extra_close_ratio=0.2, n_close_steps=20, contact_dist_threshold=0.005, hold_steps=5),
