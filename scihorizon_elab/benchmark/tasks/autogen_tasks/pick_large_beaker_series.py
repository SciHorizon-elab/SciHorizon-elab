...
@register.add_config_manager("pick_large_beaker")
class PickLargeBeakerConfigManager(BenchTaskConfigManager):
    ...
    def load_init_containers(self, init_container):
        if init_container is None or init_container == "chemistry_tube_stand":
            container_config = dict(
                ...
                position=[random.uniform(-0.3035, -0.0685), random.uniform(-0.1300, 0.3300), 0.8],
            )
    ...
    def load_objects(self, target_entity):
        ...
        obj_config = dict(
            ...
            position=[random.uniform(-0.0685, 0.0685), random.uniform(-0.0745, 0.0625), 0.8],
        )
    ...
    def get_instruction(self, target_entity, init_container, **kwargs):
        self.config["task"]["instructions"] = ["Pick one <large_beaker_0>."]
...
@register.add_task("pick_large_beaker")
class PickLargeBeakerTask(PrimitiveTask):
