...
@register.add_config_manager("pick_tube")
class PickTubeConfigManager(BenchTaskConfigManager):
    ...
    def load_init_containers(self, init_container):
        if init_container is None or init_container == "chemistry_tube_stand":
            container_config = dict(
                ...
                position=[random.uniform(-0.1050, 0.1050), random.uniform(-0.1100, 0.3600), 0.8],
            )
...
@register.add_task("pick_tube")
class PickTubeTask(PrimitiveTask):
