...
@register.add_config_manager("pick_up_flask")
class PickUpFlaskConfigManager(BenchTaskConfigManager):
    ...
    def get_instruction(self, target_entity, **kwargs):
        self.config["task"]["instructions"] = ["Pick up the <florence_flask_0>."]
...
@register.add_task("pick_up_flask")
class PickUpFlaskTask(PrimitiveTask):
    def __init__(self, task_name, robot, **kwargs):
        super().__init__(task_name, robot=robot, **kwargs)

    def get_expert_skill_sequence(self, physics):
        skill_sequence = [
            partial(SkillLib.pick, target_entity_name="florence_flask_0", prior_eulers=[[-3.14159, 0, 0]]),
        ]
