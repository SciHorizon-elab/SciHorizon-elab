...
# 区域定义（单位：m）
# 杯子: x轴负半区, x∈[-0.25,-0.05], y∈[-0.10,0.10]  (20×20cm, x轴在负侧)
# 垫子: x轴正半区, x∈[0.05,0.25],  y∈[-0.10,0.10]  (20×20cm, x轴在正侧)
# 两区域中心在x轴间隔 20cm，边缘间隔 5cm

@register.add_config_manager("place_beaker_2")
class PlaceBeaker2ConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        obj_config = dict(
            ...
            position=[random.uniform(-0.25, -0.05), random.uniform(-0.10, 0.10), 0.8],
        )
        ...
        obj_config = dict(
            ...
            position=[random.uniform(0.05, 0.25), random.uniform(-0.10, 0.10), 0.8],
        )
...
@register.add_task("place_beaker_2")
class PlaceBeaker2Task(PrimitiveTask):
