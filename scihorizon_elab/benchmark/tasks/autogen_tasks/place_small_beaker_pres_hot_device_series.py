...
@register.add_config_manager("place_small_beaker_pres_hot_device")
class PlaceSmallBeakerPresHotDeviceConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        obj_config = dict(
            ...
            position=[random.uniform(-0.3, -0.3), random.uniform(-0.05, 0.1), 0.8],
        )
    ...
    def get_instruction(self, target_entity, **kwargs):
        self.config["task"]["instructions"] = ["Place the <small_beaker_0> on the <heat_device_0>."]
...
@register.add_task("place_small_beaker_pres_hot_device")
class PlaceSmallBeakerPresHotDeviceTask(PrimitiveTask):
