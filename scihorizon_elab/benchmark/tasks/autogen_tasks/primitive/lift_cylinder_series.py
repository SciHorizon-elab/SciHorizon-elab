...
@register.add_config_manager("lift_cylinder")
class LiftCylinderConfigManager(BenchTaskConfigManager):
    ...
    def get_instruction(self, target_entity, **kwargs):
        self.config["task"]["instructions"] = ["lift the <cylinder_small_0>"]

    def get_condition_config(self, target_entity, **kwargs):
        conditions_config = dict(
            lift=dict(entities=['cylinder_small_0'], lift_height=0.15)
        )
        self.config["task"]["conditions"] = conditions_config


@register.add_task("lift_cylinder")
class LiftCylinderTask(PrimitiveTask):
