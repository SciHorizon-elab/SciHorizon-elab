...
@register.add_config_manager("unscrew_bottle")
class UnscrewBottleConfigManager(BenchTaskConfigManager):
    ...
    def get_condition_config(self, target_entity, **kwargs):
        conditions_config = [
            dict(cap_open=dict(entities=['aspirin_pill_bottle_0'])),
        ]
        self.config["task"]["conditions"] = conditions_config


@register.add_task("unscrew_bottle")
class UnscrewBottleTask(PrimitiveTask):
    def __init__(self, task_name, robot, **kwargs):
        super().__init__(task_name, robot=robot, **kwargs)

    def get_expert_skill_sequence(self, physics):
        skill_sequence = [
            partial(SkillLib.unscrew_cap, target_entity_name="aspirin_pill_bottle_0"),
        ]
