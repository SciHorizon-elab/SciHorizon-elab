...
@register.add_config_manager("put_small_beaker")
class PutSmallBeakerConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        obj_config = dict(
            ...
            position=[random.uniform(-0.05, 0.05), random.uniform(-0.15, -0.05), 0.8],
    ...
    def get_instruction(self, target_entity, **kwargs):
        self.config["task"]["instructions"] = ["put the <small_beaker_0> which contains <CuSO4 solution_0> into the <water_bath_0>."]
...
@register.add_task("put_small_beaker")
class PutSmallBeakerTask(PrimitiveTask):
    ...
    def get_expert_skill_sequence(self, physics):
        skill_sequence = [
            ...
            partial(SkillLib.lift, lift_height=0.40),  # 抬起确保从上方进入
