...
from scihorizon_elab.simulation.conditions import ConditionSet, SequentialConditionSet, OrCondition
from scihorizon_elab.utils.utils import grid_sample, distance
from scihorizon_elab.simulation.entities.scene import Scene
...
from scihorizon_elab.utils.gpt_utils import query_gpt4_v


with open(os.path.join(os.getenv("SCIHORIZON_ELAB_ROOT"), "simulation", "configs", "camera_config.json"), "r") as f:
...
class LM4ManipBaseTask(composer.Task):
    ...
    def __init__(self, 
        ...
        self.asset_path = os.path.join(os.getenv("SCIHORIZON_ELAB_ROOT"), "simulation", "assets")
    ...
    def before_step(self, physics, action, random_state):
        data = physics.data
        # ========== 重力前馈补偿(消除位置控制稳态下垂) ==========
        # 执行器模型(general, biastype=affine): force = kp*(ctrl - q) - kv*qdot。
        # 静态平衡时 kp*(ctrl-q) = τ_gravity,稳态误差 = τ_gravity/kp ≈ 0.01 rad
        # (joint2 实测),表现为技能段边界处末端"点头"式下沉(目标重置跳变)。
        # 把 qfrc_bias(重力+科氏,但准静态下以重力为主)按 kp 折算进 ctrl,
        # 平衡点移到 q = ctrl,稳态误差归零。
        # 用 DM_ENV_GRAVITY_COMP=0 可关闭(对照实验)。
        if getattr(self, "_gravity_comp_enabled", None) is None:
            self._gravity_comp_enabled = os.environ.get("DM_ENV_GRAVITY_COMP", "1") == "1"
        ctrl = np.array(action, dtype=np.float64).copy()
        if self._gravity_comp_enabled:
            qposadr = getattr(self, "_actuator_qposadr", None)
            if qposadr is None:
                # 缓存每个执行器作用的首关节 qpos 地址(仅 joint 传动;
                # tendon 传动如注释中的 actuator8 不适配,置 qposadr=-1 跳过补偿)
                from dm_control.mujoco.wrapper.mjbindings import enums
                model = physics.model
                self._actuator_qposadr = []
                self._actuator_kp = []
                for i in range(model.nu):
                    kp = model.actuator_gainprm[i][0]
                    if (model.actuator_trntype[i] == enums.mjtTrn.mjTRN_JOINT
                            and kp > 0):
                        jid = model.actuator_trnid[i][0]
                        self._actuator_qposadr.append(model.jnt_qposadr[jid])
                        self._actuator_kp.append(kp)
                    else:
                        self._actuator_qposadr.append(-1)
                        self._actuator_kp.append(kp)
            for i in range(len(ctrl)):
                qadr = self._actuator_qposadr[i]
                kp = self._actuator_kp[i]
                if kp > 0:
                    ctrl[i] += data.qfrc_bias[qadr] / kp
        data.ctrl[:] = ctrl
    ...
    def build_instruction(self):
        ...
        with open(os.path.join(os.getenv("SCIHORIZON_ELAB_ROOT"), "configs/prompt/prompt.json"), "r") as f:
            prompts = json.load(f)
