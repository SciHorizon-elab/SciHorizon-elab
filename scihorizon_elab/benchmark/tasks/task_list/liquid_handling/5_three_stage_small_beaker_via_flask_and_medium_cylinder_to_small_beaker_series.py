...
@register.add_config_manager("5_three_stage_small_beaker_via_flask_and_medium_cylinder_to_small_beaker")
class Task5ThreeStageSmallBeakerViaFlaskAndMediumCylinderToSmallBeakerConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        ...
        obj_config = dict(
            ...
            position=[random.uniform(-0.2620, -0.1250), random.uniform(0.0565, 0.1935), 0.8],
        )
        obj_config["class"] = "ChemistryContainer"
        obj_config["randomness"] = dict(pos=[0.0200, 0.0200, 0], quat=[0, 0, 0.05])
        self.config["task"]["components"].append(obj_config)
        ...
        obj_config = dict(
            name="small_beaker_1",
            xml_path=name2class_xml["small_beaker"][-1],
            position=[random.uniform(0.1250, 0.1960), random.uniform(0.0910, 0.1590), 0.8],
        )
        obj_config["class"] = "ChemistryContainer"
        obj_config["randomness"] = dict(pos=[0.0200, 0.0200, 0], quat=[0, 0, 0.05])
        self.config["task"]["components"].append(obj_config)
...
@register.add_task("5_three_stage_small_beaker_via_flask_and_medium_cylinder_to_small_beaker")
class Task5ThreeStageSmallBeakerViaFlaskAndMediumCylinderToSmallBeakerTask(PrimitiveTask):
