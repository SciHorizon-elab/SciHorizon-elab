...
@register.add_config_manager("7_pour_tube_into_large_beaker_mechanical_pipette_large_beaker_to_flask")
class Task7PourTubeIntoLargeBeakerMechanicalPipetteLargeBeakerToFlaskConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        ...
        obj_config = dict(
            ...
            position=[random.uniform(-0.1520, -0.0150), random.uniform(0.1440, 0.2810), 0.8],
            solution="KMnO4_solution_1",
            solution_rgba=[0.5, 0.0, 0.5, 0.4],
        )
        obj_config["class"] = "ChemistryContainer"
    ...
    def get_instruction(self, target_entity, init_container, **kwargs):
        self.config["task"]["instructions"] = ["Pick up the <tube_0> which contains <KMnO4 solution_0>."]
...
@register.add_task("7_pour_tube_into_large_beaker_mechanical_pipette_large_beaker_to_flask")
class Task7PourTubeIntoLargeBeakerMechanicalPipetteLargeBeakerToFlaskTask(PrimitiveTask):
