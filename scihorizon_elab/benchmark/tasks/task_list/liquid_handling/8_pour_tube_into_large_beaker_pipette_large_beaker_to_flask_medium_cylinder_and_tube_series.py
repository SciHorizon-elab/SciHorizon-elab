...
@register.add_config_manager("8_pour_tube_into_large_beaker_pipette_large_beaker_to_flask_medium_cylinder_and_tube")
class Task8PourTubeIntoLargeBeakerPipetteLargeBeakerToFlaskMediumCylinderAndTubeConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        ...
        obj_config = dict(
            ...
            solution="FeCl2 solution_1",
            solution_rgba=[0.56, 0.75, 0.45, 0.4],
            position=pos,
        )
        ...
        obj_config = dict(
            ...
            position=[random.uniform(0.0150, 0.1460), random.uniform(-0.1300, -0.0050), 0.8],
        )
        ...
        obj_config = dict(
            ...
            position=[random.uniform(0.1550, 0.2920), random.uniform(0.1050, 0.2420), 0.8],
        )
        obj_config["class"] = "ChemistryContainer"
        obj_config["randomness"] = dict(pos=[0.0200, 0.0200, 0], quat=[0, 0, 0.05])
        self.config["task"]["components"].append(obj_config)
        ...
        obj_config = dict(
            ...
            position=[random.uniform(0.0050, 0.0550), random.uniform(0.1000, 0.1500), 0.8],
        )
        obj_config["class"] = "ChemistryContainer"
    ...
    def get_instruction(self, target_entity, init_container, **kwargs):
        self.config["task"]["instructions"] = ["Pick up the <tube_0> which contains <FeCl2 solution_0>."]
    ...
    def get_condition_config(self, target_entity, init_container, **kwargs):
        conditions_config = [
            ...
            dict(aspirate=dict(source_container='large_beaker_0', tool='pipette_0', robot='robot')),
            dict(dispense=dict(target_container='flask_0', tool='pipette_0', robot='robot')),
            dict(aspirate=dict(source_container='large_beaker_0', tool='pipette_0', robot='robot')),
            dict(dispense=dict(target_container='cylinder_mid_0', tool='pipette_0', robot='robot')),
            dict(aspirate=dict(source_container='large_beaker_0', tool='pipette_0', robot='robot')),
            dict(dispense=dict(target_container='tube_1', tool='pipette_0', robot='robot')),
        ]
        self.config["task"]["conditions"] = conditions_config


@register.add_task("8_pour_tube_into_large_beaker_pipette_large_beaker_to_flask_medium_cylinder_and_tube")
class Task8PourTubeIntoLargeBeakerPipetteLargeBeakerToFlaskMediumCylinderAndTubeTask(PrimitiveTask):
    ...
    def get_expert_skill_sequence(self, physics):
        skill_sequence = [
            ...
            partial(SkillLib.dispense, target_container_name="cylinder_mid_0"),
            partial(SkillLib.aspirate, source_container_name="large_beaker_0"),
            partial(SkillLib.moveto_entity, target_entity_name="tube_1", offset=np.array([0, 0, 0.08])),
            partial(SkillLib.dispense, target_container_name="tube_1"),
