...
@register.add_config_manager("9_ordered_addition_three_stage_pipette_small_beaker_then_flask_then_mechanical_pipette_another_small_beaker_into_large_beaker")
class Task9OrderedAdditionThreeStagePipetteSmallBeakerThenFlaskThenMechanicalPipetteAnotherSmallBeakerIntoLargeBeakerConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        ...
        obj_config = dict(
            ...
            position=[random.uniform(0.0150, 0.0830), random.uniform(0.1440, 0.2150), 0.8],
        ...
        obj_config = dict(
            ...
            position=[random.uniform(0.1960, 0.3270), random.uniform(-0.1300, -0.0050), 0.8],
        )
        obj_config["class"] = "ChemistryContainer"
        obj_config["randomness"] = dict(pos=[0.0200, 0.0200, 0], quat=[0, 0, 0.05])
        self.config["task"]["components"].append(obj_config)
        ...
        obj_config = dict(
            ...
            position=[random.uniform(0.1930, 0.3300), random.uniform(0.1050, 0.2420), 0.8],
    ...
    def get_instruction(self, target_entity, init_container, **kwargs):
        self.config["task"]["instructions"] = ["Pick up <pipette_0>."]
    ...
    def get_condition_config(self, target_entity, init_container, **kwargs):
        conditions_config = [
            ...
            dict(dispense=dict(
            target_container='large_beaker_0',
            tool='pipette_0',
            robot='robot',
            dwell_steps=20,
        )),
...
@register.add_task("9_ordered_addition_three_stage_pipette_small_beaker_then_flask_then_mechanical_pipette_another_small_beaker_into_large_beaker")
class Task9OrderedAdditionThreeStagePipetteSmallBeakerThenFlaskThenMechanicalPipetteAnotherSmallBeakerIntoLargeBeakerTask(PrimitiveTask):
    ...
    def get_expert_skill_sequence(self, physics):
        skill_sequence = [
            ...
            partial(SkillLib.moveto_entity, target_entity_name="large_beaker_0", offset=np.array([0, 0, 0.08])),
            partial(SkillLib.dispense, target_container_name="large_beaker_0"),
