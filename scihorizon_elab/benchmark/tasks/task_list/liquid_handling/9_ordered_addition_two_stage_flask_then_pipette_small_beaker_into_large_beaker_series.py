...
@register.add_config_manager("9_ordered_addition_two_stage_flask_then_pipette_small_beaker_into_large_beaker")
class Task9OrderedAdditionTwoStageFlaskThenPipetteSmallBeakerIntoLargeBeakerConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        ...
        obj_config = dict(
            ...
            position=[random.uniform(-0.3270, -0.1960), random.uniform(0.1170, 0.2420), 0.8],
        )
        obj_config["class"] = "ChemistryContainer"
        obj_config["randomness"] = dict(pos=[0.0200, 0.0200, 0], quat=[0, 0, 0.05])
        self.config["task"]["components"].append(obj_config)
    ...
    def get_instruction(self, target_entity, init_container, **kwargs):
        self.config["task"]["instructions"] = ["Pick up the <flask_0> which contains <KMnO4 solution_0>."]
...
@register.add_task("9_ordered_addition_two_stage_flask_then_pipette_small_beaker_into_large_beaker")
class Task9OrderedAdditionTwoStageFlaskThenPipetteSmallBeakerIntoLargeBeakerTask(PrimitiveTask):
