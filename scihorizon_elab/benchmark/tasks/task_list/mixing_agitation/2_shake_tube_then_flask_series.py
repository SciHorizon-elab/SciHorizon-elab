...
@register.add_config_manager("2_shake_tube_then_flask")
class Task2ShakeTubeThenFlaskConfigManager(BenchTaskConfigManager):
    ...
    def load_init_containers(self, init_container):
        if init_container is None or init_container == "chemistry_tube_stand":
            container_config = dict(
                ...
                position=[random.uniform(-0.0830, 0.1520), random.uniform(-0.1300, 0.3300), 0.8],
            )
    ...
    def get_instruction(self, target_entity, init_container, **kwargs):
        self.config["task"]["instructions"] = ["Retrieve the <tube_0> containing <KMnO4 solution_0> from the <chemistry_tube_stand_0>."]

    def get_condition_config(self, target_entity, init_container, **kwargs):
        conditions_config = [
            dict(not_contain=dict(container='chemistry_tube_stand_0', entities=['tube_0'])),
...
@register.add_task("2_shake_tube_then_flask")
class Task2ShakeTubeThenFlaskTask(PrimitiveTask):
