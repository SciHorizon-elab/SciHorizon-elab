...
@register.add_config_manager("2_shake_two_tubes_then_flask")
class Task2ShakeTwoTubesThenFlaskConfigManager(BenchTaskConfigManager):
    ...
    def get_instruction(self, target_entity, init_container, **kwargs):
        self.config["task"]["instructions"] = ["Shake the <tube_0> containing <FeCl2 solution_0>."]

    def get_condition_config(self, target_entity, init_container, **kwargs):
        conditions_config = [
            dict(shake=dict(entities=['tube_0'], robot='robot')),
            dict(shake=dict(entities=['tube_1'], robot='robot')),
            dict(shake=dict(entities=['flask_0'], robot='robot')),
        ]
        self.config["task"]["conditions"] = conditions_config


@register.add_task("2_shake_two_tubes_then_flask")
class Task2ShakeTwoTubesThenFlaskTask(PrimitiveTask):
