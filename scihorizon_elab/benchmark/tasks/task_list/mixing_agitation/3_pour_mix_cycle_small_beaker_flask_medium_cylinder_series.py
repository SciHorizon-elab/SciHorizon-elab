...
@register.add_config_manager("3_pour_mix_cycle_small_beaker_flask_medium_cylinder")
class Task3PourMixCycleSmallBeakerFlaskMediumCylinderConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        obj_config = dict(
            ...
            position=[random.uniform(-0.0560, 0.0150), random.uniform(-0.0680, 0.0000), 0.8],
        ...
        obj_config = dict(
            ...
            position=[random.uniform(0.1250, 0.2620), random.uniform(0.0565, 0.1935), 0.8],
        )
        obj_config["class"] = "ChemistryContainer"
        obj_config["randomness"] = dict(pos=[0.0200, 0.0200, 0], quat=[0, 0, 0.05])
        self.config["task"]["components"].append(obj_config)
    ...
    def get_instruction(self, target_entity, **kwargs):
        self.config["task"]["instructions"] = ["Pour the <KMnO4 solution_0> from the <small_beaker_0> into the <flask_0>."]
...
@register.add_task("3_pour_mix_cycle_small_beaker_flask_medium_cylinder")
class Task3PourMixCycleSmallBeakerFlaskMediumCylinderTask(PrimitiveTask):
