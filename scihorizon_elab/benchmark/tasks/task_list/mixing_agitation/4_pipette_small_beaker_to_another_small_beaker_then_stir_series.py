...
@register.add_config_manager("4_pipette_small_beaker_to_another_small_beaker_then_stir")
class Task4PipetteSmallBeakerToAnotherSmallBeakerThenStirConfigManager(BenchTaskConfigManager):
    ...
    def load_init_containers(self, init_container):
        if init_container is None or init_container == "chemistry_tube_stand":
            container_config = dict(
                ...
                position=[random.uniform(0.0940, 0.3290), random.uniform(-0.1300, 0.3300), 0.8],
            )
    ...
    def load_objects(self, target_entity):
        ...
        obj_config = dict(
            ...
            position=[random.uniform(-0.1650, -0.0940), random.uniform(0.0910, 0.1590), 0.8],
        ...
        obj_config = dict(
            name="small_beaker_1",
            xml_path=name2class_xml["small_beaker"][-1],
            position=[random.uniform(-0.0870, -0.0160), random.uniform(-0.0870, -0.0190), 0.8],
        )
        obj_config["class"] = "ChemistryContainer"
        obj_config["randomness"] = dict(pos=[0.0200, 0.0200, 0], quat=[0, 0, 0.05])
        self.config["task"]["components"].append(obj_config)
    ...
    def get_instruction(self, target_entity, init_container, **kwargs):
        self.config["task"]["instructions"] = ["Use the <pipette_0> to aspirate <KMnO4 solution_0> from <small_beaker_0>."]
    ...
    def get_condition_config(self, target_entity, init_container, **kwargs):
        conditions_config = [
            ...
            dict(dispense=dict(
            target_container='small_beaker_1',
            tool='pipette_0',
            robot='robot',
            dwell_steps=20,
        )),
...
@register.add_task("4_pipette_small_beaker_to_another_small_beaker_then_stir")
class Task4PipetteSmallBeakerToAnotherSmallBeakerThenStirTask(PrimitiveTask):
    ...
    def get_expert_skill_sequence(self, physics):
        skill_sequence = [
            ...
            partial(SkillLib.moveto_entity, target_entity_name="small_beaker_1", offset=np.array([0, 0, 0.08])),
            partial(SkillLib.dispense, target_container_name="small_beaker_1"),
            partial(SkillLib.drop),
            partial(SkillLib.pick, target_entity_name="glass_stirring_rod_0"),
            partial(SkillLib.stir_entity_with_tool, target_container_name="small_beaker_1"),
        ]
