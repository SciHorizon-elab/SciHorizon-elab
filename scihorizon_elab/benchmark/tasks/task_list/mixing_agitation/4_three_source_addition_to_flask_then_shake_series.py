...
@register.add_config_manager("4_three_source_addition_to_flask_then_shake")
class Task4ThreeSourceAdditionToFlaskThenShakeConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        ...
        obj_config = dict(
            name="small_beaker_1",
            xml_path=name2class_xml["small_beaker"][-1],
            position=[random.uniform(-0.0830, -0.0150), random.uniform(0.1440, 0.2150), 0.8],
    ...
    def get_instruction(self, target_entity, init_container, **kwargs):
        self.config["task"]["instructions"] = ["Use the <pipette_0> to add <FeCl2 solution_0> from the <small_beaker_0> to the <flask_0>."]
...
@register.add_task("4_three_source_addition_to_flask_then_shake")
class Task4ThreeSourceAdditionToFlaskThenShakeTask(PrimitiveTask):
