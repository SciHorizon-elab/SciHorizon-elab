...
@register.add_config_manager("5_wait_then_mechanical_pipette_large_beaker_to_tube_and_shake")
class Task5WaitThenMechanicalPipetteLargeBeakerToTubeAndShakeConfigManager(BenchTaskConfigManager):
    ...
    def get_instruction(self, target_entity, init_container, **kwargs):
        self.config["task"]["instructions"] = ["Wait until a human adds the <CuSO4 solution_0> to the <large_beaker_0>."]
...
@register.add_task("5_wait_then_mechanical_pipette_large_beaker_to_tube_and_shake")
class Task5WaitThenMechanicalPipetteLargeBeakerToTubeAndShakeTask(PrimitiveTask):
