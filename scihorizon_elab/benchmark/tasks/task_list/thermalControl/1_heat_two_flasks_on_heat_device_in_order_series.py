...
@register.add_config_manager("1_heat_two_flasks_on_heat_device_in_order")
class Task1HeatTwoFlasksOnHeatDeviceInOrderConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        ...
        obj_config = dict(
            ...
            position=[random.uniform(-0.1710, 0.0830), random.uniform(0.1170, 0.2970), 0.8],
        )
        ...
        obj_config = dict(
            name="flask_1",
            xml_path=name2class_xml["flask"][-1],
            position=[random.uniform(0.1930, 0.3300), random.uniform(0.0565, 0.1935), 0.8],
            solution="FeCl2_solution_1",
            solution_rgba=[0.5, 1, 0.5, 0.4],
        )
        obj_config["class"] = "ChemistryContainer"
        obj_config["randomness"] = dict(pos=[0.0200, 0.0200, 0], quat=[0, 0, 0.05])
        self.config["task"]["components"].append(obj_config)
    ...
    def get_condition_config(self, target_entity, **kwargs):
        conditions_config = [
            ...
            dict(on=dict(entities=['flask_1'], container='heat_device_0')),
        ]
        self.config["task"]["conditions"] = conditions_config


@register.add_task("1_heat_two_flasks_on_heat_device_in_order")
class Task1HeatTwoFlasksOnHeatDeviceInOrderTask(PrimitiveTask):
