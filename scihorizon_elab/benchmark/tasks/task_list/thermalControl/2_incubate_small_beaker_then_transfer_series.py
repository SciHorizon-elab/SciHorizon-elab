...
@register.add_config_manager("2_incubate_small_beaker_then_transfer")
class Task2IncubateSmallBeakerThenTransferConfigManager(BenchTaskConfigManager):
    ...
    def load_objects(self, target_entity):
        obj_config = dict(
            ...
            position=[random.uniform(-0.4300, 0.0560), random.uniform(-0.0020, 0.3440), 0.8],
        )
        ...
        obj_config = dict(
            ...
            position=[random.uniform(0.1660, 0.3170), random.uniform(0.0245, 0.1755), 0.8],
        )
    ...
    def get_instruction(self, target_entity, **kwargs):
        self.config["task"]["instructions"] = ["Wait until a human adds <water_0> to the <water_bath_0>. Place the <small_beaker_0> containing <FeCl2 solution_0> in the <water_bath_0>. Wait for 3 seconds. Then pick the <small_beaker_0> out of the <water_bath_0>. Place the <small_beaker_0> on the <square_mat_0>."]
...
@register.add_task("2_incubate_small_beaker_then_transfer")
class Task2IncubateSmallBeakerThenTransferTask(PrimitiveTask):
