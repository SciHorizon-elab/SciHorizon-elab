"""
scihorizon_elab Agent - 主逻辑

基于 LangGraph 构建的自动化任务生成流水线

Pipeline:
  START → analyzer → normalizer → asset_manager → skill_planner → code_generator → registration → simulation → reviewer → vlm_data → END
                                                                              ↑
                                                              (注册失败: retry_router 精准回溯)
"""
...
from .state import SciHorizonAgentState
from .nodes import (
    analyzer_node,
    asset_manager_node,
)
...
def route_after_simulation(state: Dict) -> str:
    """simulation 之后的路由 - 现在总是进入 reviewer 节点"""
    # simulation 完成后总是进入 reviewer 进行审核
    return "reviewer"


def build_vlabench_agent():
    """构建 scihorizon_elab 自动化流水线 Agent"""
    logger.info("构建 scihorizon_elab Agent...")

    workflow = StateGraph(state_schema=SciHorizonAgentState)
    ...
    workflow.add_node("reviewer", reviewer_node)  # 新增：Reviewer 节点
    ...
    workflow.add_edge("simulation", "reviewer")  # 新增：simulation → reviewer
    workflow.add_edge("reviewer", "vlm_data")
    ...
    # simulation 现在总是进入 reviewer，不再使用条件路由
    ...
    logger.info("scihorizon_elab Agent 构建完成")
    return graph
