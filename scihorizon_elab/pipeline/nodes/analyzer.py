...
**CRITICAL — Complete Task Chain**: Each step's `grounded_instruction` MUST contain the **complete action phrase for that step, including all trailing modifiers and sub-actions**. Do NOT abbreviate or drop secondary clauses. For example, if the input is "Wait until a human adds water to the water bath. Place the small beaker in the water bath." then:
- step 0: grounded_instruction = "Wait until a human adds water to the <water_bath_0>."
- step 1: grounded_instruction = "Place the <small_beaker_0> in the <water_bath_0>."
Both steps must be preserved in full — downstream code joins all step instructions to form the final task description, and dropping any step makes the task description incomplete.
...
10. **Container Contains Relationship (INITIAL state only)**: For any container entity (beaker, tube, flask, etc.) that **already contains** a non-physical entity (solution, chemical, liquid) **in the INITIAL state, before any step executes**, you MUST include `"contains": "<non_physical_raw_id>"` in the container's `semantic_attributes`. Example: if the instruction says "beaker which contains CuSO4", then:
    ...
    **IMPORTANT — future additions are NOT initial contents**: If the substance will only be added **during the task** by an external agent (e.g., "wait until a human adds X to the beaker", "add X to the flask"), the container starts EMPTY — do **NOT** add `contains` to the container in that case. Still extract the substance itself as an independent entity with its `solution_color` (later steps may reference it).
...
Input: "Wait until a human adds CuSO4 solution to the small beaker. After the addition is complete, pick up the small beaker and pour the solution into the flask."

Output:
{{
    "raw_entities": [
        {{"raw_id": "beaker_1", "raw_type": "small beaker", "semantic_attributes": {{"state": "small"}}}},
        {{"raw_id": "CuSO4_solution_1", "raw_type": "CuSO4 solution", "semantic_attributes": {{"solution_color": [0, 0.45, 1, 0.4]}}}},
        {{"raw_id": "flask_1", "raw_type": "flask", "semantic_attributes": {{}}}}
    ],
    "raw_steps": [
        {{
            "step_id": 0,
            ...
            "primary_obj": null,
            "secondary_obj": null,
            "grounded_instruction": "Wait until a human adds <CuSO4_solution_1> to the <beaker_1>.",
            "semantic_attributes": {{"external_action": "human adds <CuSO4_solution_1> to <beaker_1>"}}
        }},
        {{
            "step_id": 1,
            "action": "pick up",
            "primary_obj": "beaker_1",
            "secondary_obj": null,
            "grounded_instruction": "After the addition is complete, pick up the <beaker_1>."
        }},
        {{
            "step_id": 2,
            "action": "pour",
            "primary_obj": "beaker_1",
            "secondary_obj": "flask_1",
            "grounded_instruction": "After the addition is complete, pour the <CuSO4_solution_1> into the <flask_1>."
        }}
    ]
}}
Note in this example: the beaker starts EMPTY because the CuSO4 solution is added later by a human (during the wait_for step). The beaker's semantic_attributes therefore do NOT contain "contains" — the solution is a future addition, not initial contents. The CuSO4 solution is still extracted as an entity with its solution_color for later steps to reference."""
