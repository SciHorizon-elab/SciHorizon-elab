...
import scihorizon_elab.simulation.entities  # noqa: F401
...
def _infer_class_name(spec: str, init_params: Dict) -> str:
    """
    基于 init_params 的特征工厂模式，动态推断 scihorizon_elab 底层类名。

    判断优先级:
    0. spec → 具体子类硬编码映射（如 pipette→Pipette，避免 fallback 到 CommonGraspedEntity
       后丢失 LiquidTransferMixin 能力）
    1. contains_substance → ChemistryContainer / ChemistryTube
    2. is_container → Container / ContainerWithDoor
    3. name2class_xml registry 默认类名
    4. 兜底 → CommonGraspedEntity
    """
    # ---------- 优先级 0: spec → 具体子类硬编码映射 ----------
    SPEC_TO_ENTITY_CLASS = {
        "pipette": "Pipette",
        "dropper": "Dropper",
    }
    if spec in SPEC_TO_ENTITY_CLASS:
        mapped = SPEC_TO_ENTITY_CLASS[spec]
        logger.info(f"  → 类推断: {spec} 命中 SPEC_TO_ENTITY_CLASS → {mapped}")
        return mapped
    ...
    if init_params.get("contains_substance"):
        ...
        logger.info(f"  → 类推断: {spec} + contains_substance → ChemistryContainer")
        return "ChemistryContainer"
    ...
    if init_params.get("is_container"):
        ...
        logger.info(f"  → 类推断: {spec} + is_container → Container")
        return "Container"
...
def asset_manager_node(state: Dict) -> Dict:
    ...
    for inst in instances:
        ...
        if "contains_substance" in init_params:
            ...
            if class_name in ("ChemistryContainer", "ChemistryTube", "Container"):
