...
Select from these condition types (registered in scihorizon_elab/simulation/conditions.py):
...
- **aspirate**(source_container=<uid>, tool=<uid>, robot="robot", dwell_steps=20):
  Liquid-aspirate check. The tool's aspirate_site must be in contact with the source
  container's solution geom. Latches true on first success.
  - `source_container`: the container being drawn from (e.g., small_beaker_0)
  - `tool`: the pipette/dropper being used (e.g., pipette_0)
  - `dwell_steps`: geometric dwell steps (default 20 ≈ 2s @10fps; aspirate_site must
    stay within source container XY bbox and below liquid surface for that many consecutive steps)
  Use for: "aspirate X from Y", "draw liquid from Y into X", "pipette X from Y"
  **IMPORTANT**: Always include `"robot": "robot"` in params.

- **dispense**(target_container=<uid>, tool=<uid>, robot="robot", dwell_steps=20):
  Geometric dispense check (mirrors dispense skill):
    1. tool is grasped
    2. tool's aspirate_site is inside target_container AABB
    3. tool's aspirate_site Z <= target's placepoint Z
    4. conditions 1-3 hold for `dwell_steps` consecutive steps (default 20 ≈ 2s),
       with per-frame movement < 0.5cm
  On success: triggers solution_reaction transfer (same as pour_into) and latches.
  - `target_container`: the receiving container (e.g., large_beaker_0)
  - `tool`: the pipette/dropper (e.g., pipette_0)
  Use for: "dispense X into Y", "drop solution into Y", "transfer liquid to Y"
  **IMPORTANT**: Always include `"robot": "robot"` and `"tool": <pipette_uid>` in params.
...
| aspirate (draw in) | **aspirate** | Tool's aspirate_site contacts source solution geom |
| dispense (release) | **dispense** | Target container's solution rgba changed (or matches expected) |
...
- **aspirate/dispense action**: Use the **aspirate** / **dispense** conditions. Do NOT use contain or on — these are physical state checks, not liquid transfer checks.
"""
        ...
        if cond_type != "pass":
            ...
            for k, v in params.items():
                ...
                if k in ["positions", "target_pos_range", "orientations",
                         ...
                         "min_distance", "tool_tip_site", "dwell_steps"]:
                    continue
                    continue
                # entity 参数应该是字符串 UID
