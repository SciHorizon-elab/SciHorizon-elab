...
将生成的 Python 任务文件动态导入，注册到 scihorizon_elab 的任务注册表中，
...
def registration_node(state: Dict) -> Dict:
    ...
    # 推导模块名：如果 task_module_path 包含子目录(如 autogen_tasks/experiments/ 或
    # task_list/liquid_handling/)，模块名也要相应反映路径。否则 importlib 会找不到。
    if task_module_path:
        # 取 SCIHORIZON_ELAB_ROOT/benchmark/tasks/ 之后的部分作为子路径
        vlabench_root_env = os.environ.get("SCIHORIZON_ELAB_ROOT", "")
        module_path = Path(task_module_path)
        try:
            rel_path = module_path.resolve().relative_to(Path(vlabench_root_env) / "benchmark" / "tasks")
            # rel_path 形如 "autogen_tasks/lift_flask_series.py" 或
            #              "task_list/liquid_handling/lift_flask_series.py"
            rel_no_ext = str(rel_path.with_suffix(""))
            # Python 子模块路径用 . 分隔
            sub_path = rel_no_ext.replace("/", ".")
            module_name = f"scihorizon_elab.benchmark.tasks.{sub_path}"
        except (ValueError, IndexError):
            # 文件不在 benchmark/tasks/ 下，回退到默认模块名
            module_name = f"scihorizon_elab.benchmark.tasks.autogen_tasks.{task_name}_series"
    else:
        module_name = f"scihorizon_elab.benchmark.tasks.autogen_tasks.{task_name}_series"
    ...
    try:
        ...
        # 2. 先导入 tasks 触发 @register 装饰器（避免循环导入）
        import scihorizon_elab.benchmark.tasks  # noqa: F401

        # 3. 更新 name2config（内存）
        # 同时更新 scihorizon_elab.benchmark.configs 中的和 envs 模块中的 name2config，
        ...
        if vlabench_root:
            config_json_path = Path(vlabench_root) / "benchmark" / "configs" / "task_config.json"
