...
def simulation_node(state: Dict) -> Dict:
    ...
    # 确保 SCIHORIZON_ELAB_ROOT 环境变量已设置（assets 在 scihorizon_elab 子目录下）
    if not os.environ.get("SCIHORIZON_ELAB_ROOT"):
        os.environ["SCIHORIZON_ELAB_ROOT"] = str(Path(__file__).resolve().parents[3] / "scihorizon_elab")
    ...
    try:
        ...
        import importlib
        importlib.import_module("scihorizon_elab.simulation.robots")  # 触发 @register.add_robot
        importlib.import_module("scihorizon_elab.benchmark.tasks.autogen_tasks")  # 触发 @register.add_task
        ...
        series_path = os.path.join(
            vlabench_root, "benchmark", "tasks", "autogen_tasks",
