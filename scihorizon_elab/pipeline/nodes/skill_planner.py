...
- aspirate(source_container_name, dwell_steps=20, descend_below_surface=0.01): **用手中移液管/滴管吸取源容器中的液体**。内部已封装：垂直移到容器上方 20cm → 下降到液面下 1cm → 检测 aspirate_site 与 solution geom 接触 dwell_steps 步（默认 20≈2s）→ 存储溶液信息到工具。source_container_name 是源容器 uid（如 small_beaker_0）。**任务要求"用移液管吸取/抽吸/从…中取样"时，必须使用此技能，不要拆解为 pick+moveto_entity+wait**。必须在之前先 pick 工具。
- dispense(target_container_name, dwell_steps=20): **用手中移液管/滴管向目标容器注入液体**。内部已封装：垂直移到目标上方 15cm → 下降到容器口内 2cm → 在容器 AABB 内稳定 dwell_steps 步（默认 20 ≈ 2s）→ 触发目标容器溶液显示。target_container_name 是目标容器 uid（如 large_beaker_0）。**任务要求"用移液管滴加/注入/分配液体到…"时，必须使用此技能，不要拆解为 moveto_entity+wait**。必须在 aspirate 之后使用。**严禁使用 contact_steps 参数（dispense 没有这个参数，会报错）。**
...
- **aspirate / dispense 任务的正确序列**:
  - 先 pick 移液管/滴管（pipette_0 / dropper_0），再 lift 0.15，然后调用 aspirate(source_container_name=<src_uid>)。如果任务还要把液体转移到另一容器，再 moveto_entity 到目标上方，最后 dispense(target_container_name=<dst_uid>)。
  - 完整示例: pick(pipette_0) -> lift(0.15) -> aspirate(small_beaker_0) -> moveto_entity(large_beaker_0, offset=[0,0,0.08]) -> dispense(large_beaker_0) -> drop。
"""
...
ACTION_TO_SKILL_HINT = {
    ...
    "aspirate": ["pick", "lift", "moveto_entity", "aspirate"],
    "dispense": ["aspirate", "moveto_entity", "dispense"],
