...
def _register_downloaded_asset(canonical_name: str, xml_path: str):
    ...
    Args:
        ...
        xml_path: 相对于 SCIHORIZON_ELAB_ROOT/assets/ 的 XML 路径
    """
    try:
        from scihorizon_elab.simulation.entities import CommonGraspedEntity
...
def check_asset_exists(object_name: str) -> Dict:
    ...
    try:
        ...
        logger.warning("  ⚠ scihorizon_elab.simulation.configs.constant 导入失败，跳过配置查找")
...
def download_asset(keyword: str, max_downloads: int = 3) -> Dict:
    ...
    if not vlabench_root:
        return {
            ...
            "downloaded_count": 0,
            "assets": [],
            "error": "SCIHORIZON_ELAB_ROOT 未设置"
        }

    # get_assets.py 在 scihorizon_elab/pipeline/tools/ 下
    vlabench_root = os.environ.get('SCIHORIZON_ELAB_ROOT')
    if not vlabench_root:
        return {
            "success": False,
    ...
    # SCIHORIZON_ELAB_ROOT 指向 scihorizon_elab 包内目录（如 .../scihorizon_elab/scihorizon_elab），
    # get_assets.py 在该目录的 pipeline/tools/ 下，不要再多拼一层 scihorizon_elab
    ...
    try:
        # 将资产下载到 scihorizon_elab 包内的 assets 目录
        # 因为 scihorizon_elab ���在 SCIHORIZON_ELAB_ROOT/assets/ 下查找资产
        result = subprocess.run([
            ...
            "--skip_existing"
        ], capture_output=True, text=True, timeout=600, cwd=vlabench_root)  # 在 scihorizon_elab 目录运行
