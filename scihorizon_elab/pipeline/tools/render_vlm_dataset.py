...
# 设置无头模式(在导入 scihorizon_elab 之前)
os.environ['MUJOCO_PY_NO_XR'] = '1'
os.environ['DISPLAY'] = ''
os.environ['MUJOCO_GL'] = 'osmesa'  # 使用 OSMesa 离线渲染，无需 X11 显示

# 设置环境变量(必须在导入 scihorizon_elab 之前)
if "SCIHORIZON_ELAB_ROOT" not in os.environ:
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    os.environ["SCIHORIZON_ELAB_ROOT"] = project_root

# 导入 scihorizon_elab 组件
from scihorizon_elab import robots
from scihorizon_elab import benchmark
...
def render_vlm_task(task_name, num_examples, dimension="M&T", overwrite=False):
    """渲染整个任务"""
    if os.environ['SCIHORIZON_ELAB_ROOT'].endswith('/scihorizon_elab'):
        base_root = os.path.dirname(os.environ['SCIHORIZON_ELAB_ROOT'])
    else:
        base_root = os.environ['SCIHORIZON_ELAB_ROOT']
