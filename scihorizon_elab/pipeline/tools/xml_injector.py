...
def _inject_solution_placeholder(xml_path: str, body_name: Optional[str]) -> str:
    ...
    供 LiquidDisplayMixin 在运行时修改 rgba 颜色。
    """
    with open(xml_path, "r", encoding="utf-8") as f:
        content = f.read()
...
def inject_xml(xml_path: str, class_name: str, spec: str) -> str:
    ...
    Args:
        ...
        class_name: scihorizon_elab 类名 (ChemistryContainer, Container 等)
        spec: 资产 spec 名称
    ...
    is_chemistry = class_name in ("ChemistryContainer", "ChemistryTube")
    if is_chemistry:
