...
from scihorizon_elab.utils.utils import distance, quaternion_to_euler, matrix_to_quaternion
from scihorizon_elab.simulation.entities.entity import Entity
from scihorizon_elab.simulation.entities.specific_entities.solute_reaction import (
...
class PourIntoCondition(Condition):
    ...
    def _transfer_solution(self, physics):
        ...
        # PourIntoCondition 里 source 是容器（LiquidDisplayMixin.clear_solution(physics)）。
        # DispenseCondition 里 source 是工具（LiquidTransferMixin.clear_solution(self) 无参）。
        # 用 try 容错：先尝试带 physics 的版本，失败则调用无参版本。
        if hasattr(source, "clear_solution"):
            try:
                source.clear_solution(physics)
            except TypeError:
                source.clear_solution()
...
class WaitForCondition(Condition):
    ...
    成功条件：目标实体当前未被机械臂抓夹接触（即未被抓住），且自 episode
    开始（record_initial_state）起已经过 wait_duration 秒仿真时间。

    params:
        entity: 要监控的实体
        robot: 机器人
        wait_duration: 等待时间（秒），默认 2.0（由 skill 使用，同时约束
            本条件的触发时机）
    ...
    def record_initial_state(self, physics=None):
        super().record_initial_state(physics)
        self._change_applied = False
        # 记录本 episode 的起始仿真时间，作为 wait_duration 的计时零点。
        # data.time 在每次 reset 后归零，而 record_initial_state 在 reset
        # 之后被调用，因此这里的读数就是本 episode 的计时基准。
        self._record_time = physics.data.time if physics is not None else None
        # 同一 env 反复 reset 时（如 run_task 重放、多 episode 复用），
        # 上一 episode 由 _apply_change 写入的溶液（rgba + solutes）会残留
        # 在实体上，导致新 episode 一开场烧杯就是有溶液的。add_solution
        # 类任务的语义是"向空烧杯加液"，这里恢复空杯前态；
        # solution_change_color 任务初始就应有溶液，不清空。
        if self.change_type == "add_solution" and self.entity is not None \
                and hasattr(self.entity, "clear_solution"):
            try:
                self.entity.clear_solution(physics)
            except Exception:
                pass
    ...
    def is_met(self, physics=None):
        ...
        # Gate：record_initial_state 之前不得触发。env.reset() 内部的
        # wait-step 循环会经 should_terminate_episode 调用 is_met，此时
        # 机械臂静止未抓取，若无此 gate，"人工添加溶液"会在 reset 阶段被
        # 提前施加到空烧杯上并残留整个 episode。与 ButtonPressedCondition
        # 的防误触 gate 同一模式（TROUBLESHOOTING.md 条目 #3 检查清单第 1 条）。
        if not self._initial_state_recorded:
            return False
        ...
        # wait_duration 计时：episode 开始后经过 wait_duration 秒仿真时间，
        # 且实体未被机械臂触碰，才认为"人工添加"发生。否则逐帧 condition
        # callback 会在技能第一步就触发，wait_for 技能的等待过程失去意义。
        if physics is not None and self._record_time is not None:
            if physics.data.time - self._record_time < self.wait_duration:
                return False

        # 实体未被抓住且等待时长已满，条件满足，应用环境变化
...
class OnOrientationCondition(Condition):
    ...
    def is_met(self, physics=None):
        for i, entity in enumerate(self.entities):
            ...
            from scihorizon_elab.utils.utils import quaternion_to_euler
...
class StirCondition(Condition):
    ...
    def _is_met_impl(self, physics):
        ...
        if top_site is not None and bottom_site is not None:
            ...
            place_points = self.container.get_place_points(physics)
            if place_points:
...
class DrawerOpenCondition(Condition):
    ...
    params:
        entities: Cabinet 实体列表（每个 entity 应有 self.joints 与 self.slide_rails）
...
@register.add_condition("aspirate")
class AspirateCondition(Condition):
    """
    工具从源容器中吸取溶液。
    通过几何接触判定：工具的 aspirate_site 与 source 的 solution geom 接触（dwell_steps 连续步数）。
    模仿 pour_into 的 latch 模式：一旦满足就 _met=True，不再重新检测。
    Latch 避免了"吸取完成后工具被移走"导致条件被重置误判。

    params:
        source_container: 源容器 entity（带 LiquidDisplayMixin 的容器）
        tool: 吸取工具 entity（带 LiquidTransferMixin 的 pipette/dropper）
        robot: 机器人 entity（用于 is_grasped 校验）
        dwell_steps: 连续几何落入步数阈值，默认 20（≈2s @10fps）
    """
    def __init__(self, source_container, tool, robot, dwell_steps=20):
        super().__init__()
        self.source_container = source_container
        self.tool = tool
        self.robot = robot
        self.dwell_steps = dwell_steps
        self._consecutive = 0  # 连续几何落入计数
    ...
    def is_met(self, physics):
        ...
        # ── 子条件 1：工具被夹爪抓取 ──
        grasped = (not hasattr(self.tool, "is_grasped")) or self.tool.is_grasped(physics, self.robot)
        if not grasped:
            print(f"[AspirateCondition] ✗ 子条件失败: tool 未被 grasp", flush=True)
            return False

        # ── 子条件 2：工具存在 aspirate_site ──
        aspirate_site = self.tool.get_aspirate_site() if hasattr(self.tool, "get_aspirate_site") else None
        if aspirate_site is None:
            print(f"[AspirateCondition] ✗ 子条件失败: tool 没有 aspirate_site", flush=True)
            return False

        # ── 子条件 3：solution geom 存在（用于算液面） ──
        sol_geom = self.source_container.mjcf_model.find("geom", "solution")
        if sol_geom is None:
            print(f"[AspirateCondition] ✗ 子条件失败: source 无 solution geom", flush=True)
            return False
        sol_z = physics.bind(sol_geom).xpos[2]
        half_h = physics.bind(sol_geom).size[1]
        liquid_surface_z = sol_z + half_h

        # ── 子条件 4：XY 在容器 bbox 内 ──
        tool_pos = physics.bind(aspirate_site).xpos
        tool_xy, tool_z = tool_pos[:2], tool_pos[2]
        keysites = self.source_container.key_sites(physics) if hasattr(self.source_container, "key_sites") else None
        if keysites:
            kp = np.array([physics.bind(k).xpos for k in keysites])
            min_xy, max_xy = kp[:, :2].min(axis=0), kp[:, :2].max(axis=0)
        else:
            center_xy = physics.bind(self.source_container.mjcf_model.worldbody).xpos[:2]
            min_xy = center_xy - 0.05
            max_xy = center_xy + 0.05
        in_xy = (tool_xy[0] >= min_xy[0]) and (tool_xy[0] <= max_xy[0]) and \
                (tool_xy[1] >= min_xy[1]) and (tool_xy[1] <= max_xy[1])

        # ── 子条件 5：Z 在液面下 [0, 5cm] 区间（容差放宽 1cm） ──
        in_z = (tool_z <= liquid_surface_z + 0.01) and (tool_z >= liquid_surface_z - 0.05)

        # 关键调试：每帧打印所有子条件值与累计命中
        print(
            f"[AspirateCondition] tool_xy=({tool_xy[0]:.4f},{tool_xy[1]:.4f}) "
            f"bbox=({min_xy[0]:.3f},{min_xy[1]:.3f})-({max_xy[0]:.3f},{max_xy[1]:.3f}) "
            f"in_xy={in_xy} | tool_z={tool_z:.4f} surface_z={liquid_surface_z:.4f} "
            f"depth={tool_z - liquid_surface_z:.4f} in_z={in_z} | "
            f"consecutive={self._consecutive}/{self.dwell_steps}",
            flush=True,
        )

        if in_xy and in_z:
            self._consecutive += 1
            if self._consecutive >= self.dwell_steps:
                self._met = True
                print(f"[AspirateCondition] ✓ latched after {self._consecutive} consecutive frames", flush=True)
                return True
            return False
        self._consecutive = 0
        return False
...
@register.add_condition("dispense")
class DispenseCondition(Condition):
    """
    工具将溶液注入到目标容器。
    判定方式（仿 AspirateCondition + PourIntoCondition）：
      1. 移液工具被夹爪抓取 (is_grasp)
      2. 工具在容器的 AABB 范围内（XY Z 均落入 key_sites bbox 或 placepoint bbox）
      3. 工具的 aspirate_site Z <= 目标容器的 placepoint Z
      4. 连续 dwell_steps 帧（默认 20 ≈ 2s）满足 1+2+3，且帧间位移 < 0.5cm
    全部满足后调用 _transfer_solution 走化学反应转移（与 PourIntoCondition 一致），
    然后 latch。

    params:
        target_container: 目标容器 entity（带 LiquidDisplayMixin 的容器）
        tool: 移液工具 entity（带 LiquidTransferMixin 的 pipette/dropper）
        robot: 机器人 entity（用于 is_grasp 校验）
        dwell_steps: 连续满足步数，默认 20 ≈ 2s @10fps
    """
    def __init__(self, target_container, tool, robot, dwell_steps=20):
        super().__init__()
        ...
        self.dwell_steps = dwell_steps
        self._transfer_applied = False
        self._consecutive = 0  # 连续满足步数
        self._last_pos = None  # 上一帧 aspirate_site 世界坐标
    ...
    def _get_aabb(self, physics):
        """获取目标容器的 AABB 边界（xmin,xmax,ymin,ymax,zmin,zmax）。
        严格使用 key_sites（group=3），没有则直接抛异常（与 dispense skill 一致）。
        容差：每个轴额外 ±2cm 浮动以容忍物理仿真/IK 精度误差。"""
        keysites = self.target_container.key_sites(physics) if hasattr(self.target_container, "key_sites") else None
        if keysites:
            kps = np.array([physics.bind(kp).xpos for kp in keysites])
            margin = 0.02  # 2cm 容差
            return (kps[:,0].min() - margin, kps[:,0].max() + margin,
                    kps[:,1].min() - margin, kps[:,1].max() + margin,
                    kps[:,2].min() - margin, kps[:,2].max() + margin)
        # 没有 key_sites：直接报错
        ent_name = getattr(self, "name", None) or getattr(self.target_container._mjcf_model, "model", "<unknown>")
        raise ValueError(
            f"DispenseCondition: 目标容器 '{ent_name}' 没有 key_sites（group=3），"
            "请在 XML 中添加以定义 AABB"
        )

    def _get_placepoint_z(self, physics):
        """获取目标容器 placepoint 的世界 Z。严格：没有直接抛异常。"""
        pp = self.target_container.get_place_points(physics)
        if pp:
            p = pp[0] if isinstance(pp, list) else pp
            return float(np.array(p)[2])
        ent_name = getattr(self, "name", None) or getattr(self.target_container._mjcf_model, "model", "<unknown>")
        raise ValueError(
            f"DispenseCondition: 目标容器 '{ent_name}' 没有 place point (group=2 site)，"
            "请在 XML 中添加 placepoint"
        )

    def _transfer_solution(self, physics):
        """将工具吸取的溶液转入目标容器。

        与 PourIntoCondition 不同：source 是工具（LiquidTransferMixin），
        它只存单值 solution / solution_rgba，没有 solutes 列表。
        优先用工具存的溶液信息；查反应表做化学反应；否则 fallback 到直接套用溶液 rgba。
        """
        ...
        # 工具的单值溶液（aspirate 时存进去的）
        src_solute = getattr(source, "solution", None)
        src_solutes = [src_solute] if src_solute else []
        src_rgba = getattr(source, "solution_rgba", None)

        tgt_solutes = list(getattr(target, "solutes", []))

        # 查反应表（无反应则保留原溶质）
        new_solutes, product = lookup_reaction(src_solutes, tgt_solutes)
        if not new_solutes and src_solutes:
            new_solutes = src_solutes + tgt_solutes

        # 决定渲染颜色
        if product is not None and product in SOLUTE2RGBA:
            new_rgba = list(SOLUTE2RGBA[product])
        elif src_rgba is not None:
            # 直接套用工具存的颜色（CuSO4 蓝），最稳妥——和 aspirate 的视觉一致
            new_rgba = list(src_rgba)
        elif new_solutes:
            new_rgba = resolve_color_from_solutes(
                new_solutes,
                getattr(target, "solution_rgba", None) or src_rgba
            )
        else:
            new_rgba = [1.0, 1.0, 1.0, 0.0]

        print(f"[DispenseCondition] _transfer_solution: src={src_solutes}, tgt={tgt_solutes}, "
              f"new_solutes={new_solutes}, product={product}, rgba={new_rgba}", flush=True)

        target.fill_solution(physics, source_solutes=new_solutes, source_solution_rgba=new_rgba)
        # 清空工具的溶液记录��工具只支持无参的 clear_solution）
        if hasattr(source, "clear_solution"):
            try:
                source.clear_solution(physics)
            except TypeError:
    ...
    def is_met(self, physics):
        ...
        grasped = (not hasattr(self.tool, "is_grasped")) or self.tool.is_grasped(physics, self.robot)
        if not grasped:
            print(f"[DispenseCondition] ✗ Gate 1 fail: tool 未被 grasp", flush=True)
            self._consecutive = 0
            return False

        # Gate 2: 工具存在 aspirate_site
        aspirate_site = self.tool.get_aspirate_site() if hasattr(self.tool, "get_aspirate_site") else None
        if aspirate_site is None:
            print(f"[DispenseCondition] ✗ Gate 2 fail: 无 aspirate_site", flush=True)
            self._consecutive = 0
            return False

        tool_pos = physics.bind(aspirate_site).xpos

        # Gate 3: 工具 XY 在容器 AABB 范围内（Z 不参与判定——dispense 静置时管尖
        # 在容器口附近停留即可，不要求严格落在容器高度 bbox 内）。
        aabb = self._get_aabb(physics)
        in_aabb = (aabb[0] <= tool_pos[0] <= aabb[1] and
                   aabb[2] <= tool_pos[1] <= aabb[3])

        if not in_aabb:
            print(f"[DispenseCondition] ✗ Gate 3 fail (XY-AABB): tool_pos={np.round(tool_pos,3)} "
                  f"aabb=x[{aabb[0]:.3f},{aabb[1]:.3f}] y[{aabb[2]:.3f},{aabb[3]:.3f}]",
                  flush=True)
            self._consecutive = 0
            return False

        # Gate 4: 工具 tip Z 必须已经达到 placepoint 高度（管口附近），
        # 即 tip 已经"插到容器口"。避免 moveto_entity 刚停下 tip 还在 placepoint
        # 上方时就提前累计 dwell，导致下降途中就触发溶液转移。
        placepoint_z = self._get_placepoint_z(physics)
        if tool_pos[2] > placepoint_z:
            print(f"[DispenseCondition] ✗ Gate 4 fail (tip above mouth): "
                  f"tip_z={tool_pos[2]:.4f} > placepoint_z={placepoint_z:.4f}",
                  flush=True)
            self._consecutive = 0
            return False

        # Gate 5: 帧间位移 < 1mm（连续 dwell 期间"几乎不动"）
        if self._last_pos is not None:
            move = float(np.linalg.norm(np.asarray(tool_pos) - np.asarray(self._last_pos)))
            if move > 0.001:  # 1mm
                print(f"[DispenseCondition] ✗ Gate 5 fail (move={move*1000:.2f}mm > 0.5mm), reset",
                      flush=True)
                self._consecutive = 0
                self._last_pos = np.asarray(tool_pos)
                return False
        self._last_pos = np.asarray(tool_pos)

        # 全部满足：累计
        self._consecutive += 1
        if self._consecutive % 5 == 0 or self._consecutive >= self.dwell_steps:
            print(f"[DispenseCondition] consecutive={self._consecutive}/{self.dwell_steps}",
                  flush=True)
        if self._consecutive >= self.dwell_steps:
            if not self._transfer_applied:
                self._transfer_solution(physics)
                self._transfer_applied = True
            self._met = True
            print(f"[DispenseCondition] ✓ latched after {self._consecutive} consecutive frames",
                  flush=True)
            return True
        return False
