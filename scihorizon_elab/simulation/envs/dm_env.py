...
from scihorizon_elab.utils.utils import euler_to_quaternion, expand_mask
from scihorizon_elab.utils.depth2cloud import rotMatList2NPRotMat, quat2Mat, posRotMat2Mat, PointCloudGenerator
...
class LM4ManipDMEnv(composer.Environment):
    def __init__(self, reset_wait_step=10, **kwargs):
        ...
        self.render_options = dict(
            # 图像分辨率架构约定（2026-08-28）：
            # env 统一渲染高保真原始分辨率 480x480；各 policy 客户端负责
            # resize 到自己训练时的输入尺寸（如 pi0.5=224, gr00t=224, ACT=256）。
            # 分辨率是"模型消费格式"而非环境属性——env 端定死尺寸会强迫
            # 所有模型吃同一分辨率。与 LIBERO 官方架构一致（env 256 渲染，
            # policy 客户端 resize 224）。
            # 历史：曾短暂在 env 端改 224（v4 评测），后迁移回 policy 端。
            height=480,
            width=480,
        )
        ...
        # rotvec 表示: ee_state 直接输出 axis-angle。RLDSDeltaActions 在训练 transform 端
        # 做 ±π wrap,根除 180° 边界 ±2π 假跳变对 target 的污染。env 端无需 unwrap 状态机。
    ...
    def _unwrap_ee_state(self, ee_state):
        """把 [pos3, quat4, gripper1] 的 ee_state 转成 [pos3, rotvec3, gripper1]。

        rotvec 是 SO(3) → 半径 π 实心球的唯一映射,无 ±π 多值问题。
        训练 transform (RLDSDeltaActions) 端对 delta 做 (d+π)%(2π)-π wrap,
        保证物理姿态穿越 180° 边界时 delta 干净。env 端无需 unwrap 状态机。
        """
        from scihorizon_elab.utils.utils import quaternion_to_rotvec
        pos, quat, gripper = ee_state[:3], ee_state[3:7], ee_state[-1]
    ...
    def get_observation(self, require_pcd=True):
        ...
        observation["ee_state"] = self._unwrap_ee_state(self.robot.get_ee_state(self.physics))
    ...
    def _sync_grasped_entity_pose(self):
        ...
        if tube_jnt_adr is None:
            ...
            # 实现"handle 跟着手走"。
            if hasattr(entity, 'slide_rails') and hasattr(entity, 'get_slide_handle_pos'):
                hand_pos_now = raw_d.xpos[hand_id].copy()
                # 找离 hand 最近的 slide handle
                best_id = None
                best_dist = float('inf')
                for rid, _rail in enumerate(entity.slide_rails):
                    try:
                        hpos = entity.get_slide_handle_pos(self.physics, rid)
                ...
                if best_id is not None:
                    drawer = entity.slide_rails[best_id]
            ...
            # （如 DryingBox），则锁定 door body 围绕 hinge 轴的角度跟随夹爪
