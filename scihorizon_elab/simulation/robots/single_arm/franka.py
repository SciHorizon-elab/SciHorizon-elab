...
from scihorizon_elab.simulation.robots.single_arm.base import SingleArm
from scihorizon_elab.utils.register import register
from scihorizon_elab.utils.utils import create_mesh_box, quaternion_to_matrix, compute_rotation_quaternion, normalize
...
class Franka(SingleArm):
    ...
    def get_qpos_from_ee_pos(self, physics, pos, quat=None, inplace=False, **kwargs):
        ...
        # ========== 链式热启动:用上一次 IK 解作为迭代起点,保证解分支连续 ==========
        # dm_control 的 qpos_from_site_pose 是牛顿迭代,从 physics.data.qpos 出发,
        # 且官方 TODO 声明不考虑关节限位(inverse_kinematics.py: "This does not take
        # joint limits into consideration")。7-DOF 冗余臂同一末端位姿有多个解分支,
        # 当肘部(joint4)逼近关节上限(-0.0698,紧邻肘伸直奇异点)时,迭代的吸引域
        # 碎裂,解会跳到另一分支(实测 joint2 单 waypoint 跳 ±0.27 rad ≈ 15°),
        # 在重力补偿使追踪误差归零后表现为末端剧烈甩动。
        # 修复:迭代起点固定为"上一个 waypoint 的解"而非当前物理 qpos——上一个解
        # 必然在正确分支上,牛顿法沿分支连续滑动,不会跨分支跳变。失败(分支湮灭/
        # 不收敛)时回退到旧行为(从当前 physics 出发)。
        prev_sol = getattr(self, "_ik_prev_solution", None)
        ik_physics = physics
        used_seed = False
        if prev_sol is not None and not inplace:
            try:
                seed_physics = physics.copy(share_model=True)
                cur_qpos = np.array(seed_physics.data.qpos, copy=True)
                n = len(prev_sol)
                cur_qpos[:n] = prev_sol
                seed_physics.data.qpos[:] = cur_qpos
                ik_physics = seed_physics
                used_seed = True
            except Exception:
                ik_physics = physics
                used_seed = False
        ik_result = qpos_from_site_pose(ik_physics,
        ...
        arm_qpos = target_qpos[:self.n_dof]
        # 成功且用了种子:更新解链;未用种子(首次/inplace/异常)不更新,
        # 避免把"从当前物理状态解出的潜在跨分支解"污染链头。
        if success and used_seed:
            self._ik_prev_solution = np.array(arm_qpos, copy=True)
        return success, arm_qpos
