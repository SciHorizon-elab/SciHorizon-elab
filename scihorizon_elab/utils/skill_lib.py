...
import mujoco
from scihorizon_elab.utils.utils import find_keypoint_and_prepare_grasp, distance, quaternion_to_euler, quaternion_to_rotvec, euler_to_quaternion, quaternion_from_axis_angle, quaternion_multiply
from scihorizon_elab.simulation.algorithms.motion_planning.rrt import rrt_motion_planning
from scihorizon_elab.simulation.algorithms.utils import interpolate_path, qauternion_slerp

logger = logging.getLogger(__name__)

# 诊断开关:SKILL_LIB_DEBUG_QPOS=1 时,step_trajectory 逐 waypoint 打印
# IK 目标跳变(tgt_jump)与逐步实际-目标关节误差(err),用于定位"俯仰关节点头"。
_DEBUG_QPOS = os.environ.get("SKILL_LIB_DEBUG_QPOS", "0") == "1"
...
class SkillLib:
    ...
    def step_trajectory(env,
        ...
        Input:
            ...
            max_n_substep: int, max number of substep for each step
                (默认 5 步而非 1 步,让 PD 控制器在每个 waypoint 有时间收敛重力下垂)
            tolerance: float, tolerance for the qpos error between the target and the current qpos
            settle_steps: int, 每个 waypoint 通过 tolerance 后额外跑 settle 步数。
                关键:settle 步的 action 固定为当前 IK 解算的目标(不重读 qpos),
                这样重力下垂不会被当作新目标,避免下一段轨迹从"下垂态"出发形成可见点头。
        Return:
        ...
        prev_target_q = None  # 诊断:上一个 waypoint 的 IK 目标
        ...
        for i, (point, quat) in enumerate(zip(points, quats)):
            ...
            waypoint = np.concatenate([point, quaternion_to_rotvec(quat), gripper_state])

            is_final_waypoint = (i == len(points) - 1)
            # 段尾用更紧的容差 + 更长的收敛预算:诊断数据显示段尾常带 ~0.02 rad
            # (约 1.1°)的残余误差进入下一段,新段目标链方向变化时误差缝瞬间
            # 拉大 0.007~0.015 rad,表现为末端"点头"。强制段尾收敛消除该误差缝。
            wp_tolerance = 0.005 if is_final_waypoint else tolerance
            wp_max_substep = max(max_n_substep, 50) if is_final_waypoint else max_n_substep

            for substep_idx in range(wp_max_substep):
                ...
                if qpos_max_error < wp_tolerance and qpos_min_error > -wp_tolerance:
                    break

            # settle 子循环:固定 action 不重读 qpos,让 PD 控制器把重力下垂收敛到稳态。
            # 避免后续 IK 解算时把"已下垂"的位置当作起点,导致俯仰关节在 waypoint
            # 切换瞬间出现可见的"点头"。
            settle_action = action
            settle_n = settle_steps if not is_final_waypoint else max(settle_steps, 10)
            for _ in range(settle_n):
                ts = env.step(settle_action)
                if ts.last():
                    task_success = True
                    break
            ...
            if _DEBUG_QPOS:
                ...
                tgt = np.array(action[:7])
                jump = tgt - prev_target_q if prev_target_q is not None else np.zeros(7)
                err = cur - tgt
                tag = " FINAL" if is_final_waypoint else ""
                print(f"[QPOS-DIAG] wp={i:4d}{tag} "
                      f"tgt={np.round(tgt,3)} cur={np.round(cur,3)} "
                      f"err={np.round(err,4)} |err|={np.linalg.norm(err):.4f} "
                      f"j2_err={err[1]:+.4f} j2_jump={jump[1]:+.4f}")
                prev_target_q = tgt.copy()
    ...
    def moveto_entity(env, target_entity_name, offset=None, gripper_state=None, **kwargs):
        ...
        目标位置计算：
            XY  = entity.get_xpos() 的水平分量（实体世界 XY 中心）
            Z   = entity.get_xpos()[2] + 抓取物 dz + 0.08 安全余量

        注意：必须夹着东西才能用本技能；如果没有抓取物 dz，直接报错。
        实体高度估算沿用 entity.get_xpos()[2]，对 body 原点在底部的容器（如 tube）
        来说 z 会偏低；调用方应保证目标 Z 至少高于物体底部 + dz + 8cm。如果后续
        需要更精确的"世界最高点"，再扩展为 geom_aabb 算法。

        姿态：强制竖直向下 euler=(-π, 0, 0)，与 dispense/insert_to_entity 一致。

        Args:
            target_entity_name: 目标实体名
            offset: 已废弃，保留仅为向后兼容（不再使用，按上述公式计算目标位置）
            gripper_state: 夹爪状态，None 则自动选择（lock 模式或当前状态）
            **kwargs: 透传给 moveto
        """
        if offset is not None:
            print(f"[moveto_entity] 警告: offset 参数已废弃，当前按 entity Z + dz + 0.08 计算目标位置")

        entity = env.task.entities[target_entity_name]
        entity_xpos = np.array(entity.get_xpos(env.physics))

        # 抓取物高度 dz：必须夹着东西，否则直接报错（不静默 fallback）
        _, grasped_entities = env.get_grasped_entity()
        if not grasped_entities:
            raise RuntimeError(
                f"[moveto_entity] 目标 '{target_entity_name}': 必须夹着工具才能 "
                f"moveto_entity（无法估算 dz）。如需无抓取悬停，请直接用 moveto。"
            )
        grasped_dz = SkillLib._get_grasped_bbox_dz(grasped_entities)

        hover_offset = grasped_dz + 0.08  # 8cm 安全余量
        target_pos = np.array([entity_xpos[0], entity_xpos[1],
                               entity_xpos[2] + hover_offset])

        # 强制竖直向下，与 dispense/insert_to_entity 保持一致
        vertical_quat = euler_to_quaternion(-np.pi, 0, 0)

        print(f"[moveto_entity] target={target_entity_name}, entity_xpos={np.round(entity_xpos, 3)}, "
              f"grasped_dz={grasped_dz:.4f}m, hover_offset={hover_offset:.4f}m, "
              f"target_pos={np.round(target_pos, 3)}")

        return SkillLib.moveto(env, target_pos, target_quat=vertical_quat,
                               gripper_state=gripper_state, **kwargs)
    ...
    def pick(env, 
        ...
        if target_pos is None or target_quat is None:
            key_pos, prepare_key_pos, key_quat = find_keypoint_and_prepare_grasp(env, target_entity, prior_eulers, specific_keypoint_id=specific_keypoint)
        ...
        if env.task.entities[target_entity_name].is_grasped(env.physics, env.robot):
            ...
            if hasattr(env, "_grasp_lock_mode") and env._grasp_lock_mode > 0:
                ...
                # 等几帧让 lock 同步稳定。
                # 关键:循环外固定一次目标 qpos(action),不要每帧重读 current qpos,
                # 否则重力/接触导致的微小下垂会被当作新目标,形成正反馈漂移。
                # 这与 aspirate/dispense 中 hold_qpos 模式保持一致。
                _lock_arm_qpos = np.array(env.robot.get_qpos(env.physics))
                _lock_action = np.concatenate([_lock_arm_qpos, env._lock_gripper_state])
                for wait_frame in range(5):
                    env.step(_lock_action)
    ...
    def place(env,
        ...
        if target_pos is None:
            place_points = target_container.get_place_points(env.physics)
            if not place_points:
    ...
    def drop(env,
        ...
        # 注意:只能抬高 path[1:],不能改 path[0]。path[0] 是机械臂当前 EE 位置,
        # 若一并抬高,第一个 waypoint 的目标 = 当前位置 + Δz,产生瞬时段跃
        # (实测 Δz=0.146m 时 joint4 单步请求 +0.67 rad,表现为末端猛然上甩)。
        grasped_names, grasped_entities = env.get_grasped_entity()
        if grasped_entities:
            grasped_entity = grasped_entities[0]
            obj_bottom_z = np.array(grasped_entity.get_xpos(env.physics))[2]
            ...
            print(f"  z_offset: {z_offset:.4f}m, 抬高量: {max(0, -z_offset):.4f}m (仅 path[1:])")
    ...
    def open_door(env,
        ...
        pick_obs, pick_wp, pick_success, _ = SkillLib.pick(
            ...
            # prior_eulers=[[0, np.pi/2, np.pi/2]],  # 俯抓 yaw=π/2：手指沿 X 轴水平开合，适配门把手朝向
        ...
        # ========== 纯接触开门 ==========
        # 夹爪末端始终垂直于门板：每步绕 hinge axis 同向旋转 delta_angle，
        # 与 get_open_trajectory 的角度增量同步，保证夹爪姿态跟随门板。
        # start_quat 取 pick 之后的当前夹爪姿态。
        start_pos, start_quat = env.robot.get_end_effector_pos(env.physics), env.robot.get_end_effector_quat(env.physics)
        ...
        # 夹爪姿态全程保持 pick 之后的朝向，不绕 world 轴旋转。
        # 沿圆弧切线方向推动把手时，夹爪自然逐渐从侧面变成垂直于门板，
        # 而不需要 IK 去求"夹爪绕 world Z 旋转"这个可能不存在的关节配置。
        trajectory_quats = [start_quat] * len(trajectory)
        stage_success = False
        task_success = False
        ...
        observations.extend(new_obs)
        waypoints.extend(new_waypoints)
        observations.pop(-1)  # step_trajectory: waypoint 先追加 obs 后追加，last break 时多一个 waypoint
        assert len(observations) == len(waypoints), f"observations and waypoints should have the same length, {len(observations)} and {len(waypoints)}"
    ...
    def open_drawer(env, 
        ...
        # open drawer   
        start_pos, start_quat = env.robot.get_end_effector_pos(env.physics), env.robot.get_end_effector_quat(env.physics)
        trajectory = target_container.get_slide_trajectory(env.physics, drawer_id)
        trajectory_quats = [start_quat for _ in range(len(trajectory))]
    ...
    def press(env, target_pos, target_quat=None, move_vector=[0, 0, 0.1], max_n_substep=100): #TODO move vector to determine the press direction
        ...
        for i in range(10):
            ...
            waypoints.append(np.concatenate([env.robot.get_end_effector_pos(env.physics),
                                             quaternion_to_rotvec(env.robot.get_end_effector_quat(env.physics)),
                                             gripper_state]))
    ...
    def pour_to_entity(env, target_container_name, tilt_angle=np.pi*2/3, tilt_velocity=np.pi/80,
        ...
        Args:
            ...
            tilt_velocity: 每步倾斜角速度（默认 π/80，较慢以减小倾倒时的离心滑出）
    ...
    def lift(env, target_pos=None, target_quat=None, gripper_state=None, lift_height=0.3, tolerance=0.02, **kwargs):
        ...
        Args:
            tolerance: qpos tolerance for step_trajectory (default 0.02m)
        """
        start_pos, start_quat = env.robot.get_end_effector_pos(env.physics), env.robot.get_end_effector_quat(env.physics)
    ...
    def reset(env, max_n_substep=200, tolerance=0.01):
        ...
        for _ in range(max_n_substep):
            ...
            env.step(action)
            obs = env.get_observation()
            pos, euler = np.array(env.robot.get_end_effector_pos(env.physics)), quaternion_to_rotvec(env.robot.get_end_effector_quat(env.physics))
    ...
    def _get_grasped_bbox_dz(grasped_objs, default_dz=0.15):
        """
        从被抓物体 XML 第二行的 @bbox 注释读取其高度 dz（如 <!-- @bbox dx=0.008 dy=0.008 dz=0.200 -->）。
        读不到时返回 default_dz。
        """
        if not grasped_objs:
            return default_dz
        entity_obj = grasped_objs[0]
        relative_xml_path = getattr(entity_obj, "relative_xml_path", None)
        if not relative_xml_path:
            print(f"[insert_to_entity] 被抓物体无 relative_xml_path，使用默认 dz={default_dz}")
            return default_dz
        xml_path = os.path.join(os.getenv("SCIHORIZON_ELAB_ROOT"), "simulation", "assets", relative_xml_path)
        if not os.path.exists(xml_path):
            print(f"[insert_to_entity] XML 不存在: {xml_path}，使用默认 dz={default_dz}")
            return default_dz
        with open(xml_path, "r") as f:
            for line in f:
                if "@bbox" in line:
                    match = re.search(r"dz=([\d.]+)", line)
                    if match:
                        dz = float(match.group(1))
                        print(f"[insert_to_entity] 被抓物体 bbox dz={dz:.4f}m (from {relative_xml_path})")
                        return dz
                stripped = line.strip()
                if stripped and not (stripped.startswith("<?") or stripped.startswith("<!--")):
                    # 只扫文件头部声明/注释块（@bbox 在这里），遇到正式 XML 内容即停止
                    break
        print(f"[insert_to_entity] {relative_xml_path} 中未找到 @bbox 注释，使用默认 dz={default_dz}")
        return default_dz
    ...
    def insert_to_entity(env, target_entity_name, insert_depth=0.05, gripper_state=None):
        ...
        task_success = False
        entity = env.task.entities[target_entity_name]
        place_points = entity.get_place_points(env.physics)
        ...
        # 1. RRT 移动到孔位上方，高度 = 被抓物体的 bbox 高度(dz) + 3cm，指定竖直向下姿态
        # dz 从被抓物体 XML 第二行的 @bbox 注释中读取（如 dz=0.200）
        grasped_dz = SkillLib._get_grasped_bbox_dz(grasped_objs)
        hover_offset = grasped_dz + 0.08
        hover_pos = insert_point + np.array([0, 0, hover_offset])
        ...
        # hover_pos = insert_point + [0,0,hover_offset]
        # 需要下降到 insert_point - [0,0,insert_depth]
        # 总下降距离 = hover_offset + insert_depth
        descend = -insert_depth
    ...
    def stir_entity_with_tool(env, target_container_name, stir_radius=0.01, stir_duration=2, insert_ratio=2/3):
        ...
        place_points = container.get_place_points(env.physics)
        if not place_points:
            return [env.get_observation()], [], False, False
    ...
    def aspirate(env,
                 ...
                 dwell_steps=20,
                 descend_below_surface=0.02,
                 gripper_state=None):
        """
        Aspirate (吸取) — 将夹持的滴管/移液管的 aspirate_site 插入源容器液面下方，
        静置 dwell_steps 步（约2s）后拔出，并把溶液信息记录到工具实体上。

        液面高度由容器 XML 中 solution geom (cylinder) 的实际尺寸计算：
            size = "radius halfheight" -> 液面 Z = geom世界Z + halfheight

        Args:
            env: LM4manipEnv object
            source_container_name: 源容器实体名
            dwell_steps: 液面下静置步数（默认20步 ≈ 2s @10fps）
            descend_below_surface: aspirate_site 低于液面的深度（m，默认2cm）
            gripper_state: 夹爪状态，默认使用 lock 模式或当前状态
        Returns:
            (observations, waypoints, stage_success, task_success)
            task_success 恒为 False（中间技能）
        """
        ...
        # ── 1. 读取容器 solution geom，计算液面高度 ──
        solution_geom = source.mjcf_model.find("geom", "solution")
        if solution_geom is None:
            print(f"[aspirate] 容器 '{source_container_name}' 没有 solution geom，无法确定液面高度")
            return observations, waypoints, False, False
        geom_z = env.physics.bind(solution_geom).xpos[2]
        half_height = env.physics.bind(solution_geom).size[1]  # cylinder: size = [radius, halfheight]
        liquid_surface_z = geom_z + half_height
        print(f"[aspirate] 容器 '{source_container_name}': solution geom z={geom_z:.4f}, "
              f"half_height={half_height:.4f} -> 液面高度 {liquid_surface_z:.4f}m")

        # ── 2. 获取工具的 aspirate_site，没有则报错终止 ──
        grasped_names, grasped_entities = env.get_grasped_entity()
        if not grasped_entities:
            print("[aspirate] 夹爪中没有抓取任何工具，无法吸取")
            return observations, waypoints, False, False
        tool = grasped_entities[0]
        aspirate_site = tool.get_aspirate_site()
        if aspirate_site is None:
            print(f"[aspirate] 工具 '{grasped_names[0]}' 没有 aspirate_site（且没有 bottom_site 兜底），"
                  "请在工具 XML 中添加 aspirate_site")
            return observations, waypoints, False, False
        aspirate_pos = np.array(env.physics.bind(aspirate_site).xpos)
        ee_pos = np.array(env.robot.get_end_effector_pos(env.physics))
        # aspirate_site 相对 EE 的 Z 偏移（后续所有目标高度用 EE_Z = 目标Z - offset 换算）
        tool_z_offset = aspirate_pos[2] - ee_pos[2]
        print(f"[aspirate] 工具 '{grasped_names[0]}': aspirate_site={np.round(aspirate_pos, 3)}, "
              f"tool_z_offset={tool_z_offset:.4f}m")

        # ── 3. 移动到容器 placepoint 正上方（与 insert_to_entity 相同逻辑：dz + 8cm） ──
        place_points = source.get_place_points(env.physics)
        insert_point = np.array(place_points[0]) if isinstance(place_points, list) else np.array(place_points)
        grasped_dz = SkillLib._get_grasped_bbox_dz(grasped_entities)
        hover_offset = grasped_dz + 0.08
        hover_pos = insert_point + np.array([0, 0, hover_offset])
        vertical_quat = euler_to_quaternion(-np.pi, 0, 0)  # 末端竖直向下
        print(f"[aspirate] 悬停目标: placepoint {np.round(insert_point, 3)} 上方 {hover_offset:.4f}m "
              f"(dz={grasped_dz:.4f} + 8cm)")
        ...
        # ── 4. 垂直下降，直到 aspirate_site 到液面下方 descend_below_surface ──
        ee_target_z = (liquid_surface_z - descend_below_surface) - tool_z_offset
        descend_lift = ee_target_z - hover_pos[2]
        print(f"[aspirate] 下降: aspirate_site 目标Z={liquid_surface_z - descend_below_surface:.4f} "
              f"(液面下{descend_below_surface*100:.1f}cm), EE目标Z={ee_target_z:.4f}, 下降量={descend_lift:.4f}m")
        obs, wp, _, _ = SkillLib.lift(env, lift_height=descend_lift, gripper_state=gripper_state, tolerance=0.005)
        observations.extend(obs)
        waypoints.extend(wp)

        # ── 5. 静置 dwell_steps 步（约2s），模拟吸取 ──
        # 关键：循环外固定一次目标 qpos。若每步重读当前 qpos，重力/接触导致的微小
        # 下垂会被当作新目标，形成正反馈漂移（夹爪向底座方向滑动）。
        hold_qpos = np.array(env.robot.get_qpos(env.physics)).reshape(-1)
        hold_action = np.concatenate([hold_qpos, gripper_state])
        for _ in range(dwell_steps):
            timestep = env.step(hold_action)
            if timestep.last():
                task_success = True
                break
            waypoints.append(np.concatenate([
            ...
            observations.append(env.get_observation())
        tip_z = np.array(env.physics.bind(aspirate_site).xpos)[2]
        print(f"[aspirate] 静置完成: aspirate_site Z={tip_z:.4f}, 液面 Z={liquid_surface_z:.4f}, "
              f"相对液面 {tip_z - liquid_surface_z:.4f}m")

        # ── 6. 记录转移的溶液信息到工具实体 ──
        solution = source.solutes[0] if getattr(source, 'solutes', None) else getattr(source, 'solution', None)
        solution_rgba = source.get_solution_rgba(env.physics) if hasattr(source, 'get_solution_rgba') else None
        tool.store_solution(solution, solution_rgba)
        print(f"[aspirate] 已记录溶液: {solution}, rgba={solution_rgba}")
        stage_success = True

        # ── 7. 拔出：抬回到 placepoint 上方 dz+8cm 的安全悬停高度 ──
        safe_target_z = insert_point[2] + grasped_dz + 0.08
        retract_lift = safe_target_z - np.array(env.robot.get_end_effector_pos(env.physics))[2]
        obs, wp, _, _ = SkillLib.lift(env, lift_height=retract_lift, gripper_state=gripper_state)
        observations.extend(obs)
        waypoints.extend(wp)
    ...
    def dispense(env,
                 ...
                 gripper_state=None):
        """
        Dispense (点样) — move held pipette/dropper above target container's
        placepoint, descend until aspirate_site is 1cm below placepoint, dwell 2s.

        溶液转移由 DispenseCondition 在满足判定条件后调用 solution_reaction
        完成（参考 PourIntoCondition 的 _transfer_solution）。本函数只负责
        机械臂运动，不直接调 set_solution_rgba/fill_solution。

        Args:
            env: LM4manipEnv object
            target_container_name: name of the target container entity
            dwell_steps: 静置步数（默认 20 ≈ 2s @10fps）
            gripper_state: 夹爪状态，默认使用 lock 模式或当前状态
        Returns:
            observations: list of obs
            waypoints: list of waypoints
            stage_success: bool, 是否完成运动（成功到底并 dwell 完毕）
        ...
        # ── 1. 必须有 placepoint，没有就失败（不 fallback） ──
        place_points = target.get_place_points(env.physics)
        if not place_points:
            print(f"[dispense] {target_container_name} 没有 place point，dispense 失败")
            return [env.get_observation()], [], False, False
        insert_point = np.array(place_points[0]) if isinstance(place_points, list) else np.array(place_points)

        # ── 2. 获取工具 aspirate_site 和 tool_z_offset ──
        grasped_names, grasped_entities = env.get_grasped_entity()
        if not grasped_entities:
            print(f"[dispense] 夹爪中没有抓取任何工具，无法 dispense")
            return [env.get_observation()], [], False, False
        # 严格校验：必须是带 aspirate_site 的工具实体，不能拿错（比如夹到容器）
        tool = None
        for name, cand in zip(grasped_names, grasped_entities):
            if hasattr(cand, "get_aspirate_site") and cand.get_aspirate_site() is not None:
                tool = cand
                print(f"[dispense] 选工具: {name}")
                break
        if tool is None:
            print(f"[dispense] 抓取物 {grasped_names} 都没有 aspirate_site，无法 dispense")
            return [env.get_observation()], [], False, False
        aspirate_site = tool.get_aspirate_site()
        if aspirate_site is None:
            print(f"[dispense] 工具 {tool} 没有 aspirate_site，无法 dispense")
            return [env.get_observation()], [], False, False

        ee_pos = np.array(env.robot.get_end_effector_pos(env.physics))
        aspirate_pos = np.array(env.physics.bind(aspirate_site).xpos)
        tool_z_offset = aspirate_pos[2] - ee_pos[2]
        print(f"[dispense] tool={grasped_names[0]}, tool_z_offset={tool_z_offset:.4f}m")

        # ── 3. 移动到 placepoint 正上方 dz + 8cm ──
        grasped_dz = SkillLib._get_grasped_bbox_dz(grasped_entities)
        hover_offset = grasped_dz + 0.08
        hover_pos = np.array([insert_point[0], insert_point[1], insert_point[2] + hover_offset])
        vertical_quat = euler_to_quaternion(-np.pi, 0, 0)
        print(f"[dispense] target_entity={target_container_name}, placepoint={np.round(insert_point, 3)}, "
              f"hover_pos={np.round(hover_pos, 3)}, hover_offset={hover_offset:.4f}m")
        obs, wp, stage_success_mv, _ = SkillLib.moveto(
            env, target_pos=hover_pos, target_quat=vertical_quat, gripper_state=gripper_state)
        observations.extend(obs)
        waypoints.extend(wp)
        ...
        # ── 4. 下降到 aspirate_site Z = placepoint Z - 1cm ──
        tip_target_z = insert_point[2] - 0.01
        ee_target_z = tip_target_z - tool_z_offset
        descend_lift = ee_target_z - hover_pos[2]
        print(f"[dispense] 下降: aspirate_site 目标Z={tip_target_z:.4f} "
              f"(placepoint下方1cm), EE目标Z={ee_target_z:.4f}, 下降量={descend_lift:.4f}m")
        obs, wp, _, _ = SkillLib.lift(env, lift_height=descend_lift, gripper_state=gripper_state, tolerance=0.005)
        observations.extend(obs)
        waypoints.extend(wp)

        # ── 5. 静置 dwell_steps 步（约2s）。循环外固定 qpos 防止重力漂移 ──
        hold_qpos = np.array(env.robot.get_qpos(env.physics)).reshape(-1)
        hold_action = np.concatenate([hold_qpos, gripper_state])
        for _ in range(dwell_steps):
            timestep = env.step(hold_action)
            ...
            observations.append(env.get_observation())
        tip_z = np.array(env.physics.bind(aspirate_site).xpos)[2]
        print(f"[dispense] 静置完成: aspirate_site Z={tip_z:.4f}, placepoint Z={insert_point[2]:.4f}, "
              f"相对 placepoint {tip_z - insert_point[2]:.4f}m")

        # ── 6. 拔出：抬回 placepoint 上方 dz+8cm 悬停高度 ──
        safe_target_z = insert_point[2] + hover_offset
        retract_lift = safe_target_z - np.array(env.robot.get_end_effector_pos(env.physics))[2]
        obs, wp, _, _ = SkillLib.lift(env, lift_height=retract_lift, gripper_state=gripper_state)
        observations.extend(obs)
        waypoints.extend(wp)
