...
def quaternion_to_rotvec(quat):
    """将本项目约定 (w, x, y, z) 四元数转为 axis-angle / rotation vector。

    无 ±π 边界多值问题（rotvec 是 SO(3) → 半径 π 实心球的唯一映射，
    边界球面上 +π·n̂ 与 -π·n̂ 同一旋转,但 wrap 在 DeltaActions 端处理）。
    用于替换 waypoint 拼接中的 quaternion_to_euler,根除 ±π 假跳变。
    """
...
def rotvec_to_quaternion(rotvec):
    """将 axis-angle / rotation vector 转换为本项目约定 (w, x, y, z) 四元数。

    返回值供 MuJoCo IK / dm_env.get_qpos_from_ee_pos 等下游消��——这些接口
    历史约定是 wxyz,直接塞 scipy 默认的 xyzw 会引入静默的 90° 错姿态。
    """
