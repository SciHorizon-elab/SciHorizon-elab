#!/usr/bin/env python3
"""
模型注册脚本 - 将处理好的模型注册到 scihorizon_elab 系统
...
Usage:
    python scripts/register_model.py \\
        --model_dir scihorizon_elab/assets/review/flask/flask \\
...
_PROJECT_ROOT = _SCRIPTS_DIR.parent.parent.parent  # tests -> pipeline -> scihorizon_elab
...
def register_to_constant(name: str, class_name: str, xml_rel_path: str):
    ...
    - 如果 name 不存在 → 新增条目
    """
    constant_path = _PROJECT_ROOT / "scihorizon_elab" / "configs" / "constant.py"
...
def compute_xml_rel_path(xml_path: Path) -> str:
    ...
    assets_dir = _PROJECT_ROOT / "scihorizon_elab" / "assets"
    try:
...
def main():
    parser = argparse.ArgumentParser(description="注册处理好的模型到 scihorizon_elab 系统")
