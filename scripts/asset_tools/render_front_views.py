#!/usr/bin/env python3
"""
渲染 scihorizon_elab assets/review 中所有模型的正面照。
用法:
    python scripts/render_front_views.py \\
        --input-dir  scihorizon_elab/assets/review \\
...
def parse_args():
    p = argparse.ArgumentParser(description="Render front-view PNGs for scihorizon_elab review assets.")
    p.add_argument(
        "--input-dir",
        type=Path,
        default=Path(__file__).parent.parent / "scihorizon_elab" / "assets" / "review",
        help="Directory containing review assets (default: scihorizon_elab/assets/review)",
    )
    p.add_argument(
        "--output-dir",
        type=Path,
...
def _strip_default_rgba(xml: str) -> str:
    ...
    The scihorizon_elab MJCF files have <default class="visual"> with rgba="0.5 0.5 0.5 1"
