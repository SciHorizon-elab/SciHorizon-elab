...
工作流程:
  ...
  3. 可选: 调用 scihorizon_elab/pipeline/tools/render_vlm_dataset.py 渲染图像

前置条件:
  - 任务的 series 文件已创建 (scihorizon_elab/tasks/autogen_tasks/primitive/{task}_series.py)
...
# 设置环境变量（必须在导入 scihorizon_elab 之前）
os.environ.setdefault("SCIHORIZON_ELAB_ROOT", str(PROJECT_ROOT / "scihorizon_elab"))
...
def render_task_images(task_name: str, dimension: str, overwrite: bool = False) -> bool:
    ...
    render_script = Path(os.environ.get("SCIHORIZON_ELAB_ROOT", str(SCRIPT_DIR.parent))) / "pipeline" / "tools" / "render_vlm_dataset.py"
...
def list_registered_tasks():
    ...
    import scihorizon_elab.simulation.robots
    import scihorizon_elab.benchmark.tasks.autogen_tasks.primitive
    import scihorizon_elab.benchmark.tasks.autogen_tasks.composite
  ...
  任务的 series 文件必须已创建并通过调试验证。
    ...
    if args.render and success_count > 0:
        if render_task_images(task_name, dimension, args.overwrite):
            ...
            print("\n渲染失败，请手动运行:")
            print(f"  python scihorizon_elab/pipeline/tools/render_vlm_dataset.py --task {task_name} --dimension \"{dimension}\"")

    elif success_count > 0 and not args.render:
        print("下一步:")
        print(f"  1. 渲染: python scihorizon_elab/pipeline/tools/render_vlm_dataset.py --task {task_name} --dimension \"{dimension}\"")
