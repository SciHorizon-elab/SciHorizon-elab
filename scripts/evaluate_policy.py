import os
import argparse
from scihorizon_elab.benchmark.evaluation.evaluator import Evaluator
from scihorizon_elab.benchmark.evaluation.model.policy.openvla import OpenVLA
from scihorizon_elab.benchmark.evaluation.model.policy.base import RandomPolicy
from scihorizon_elab.benchmark.tasks import *
from scihorizon_elab.simulation.robots import *
...
def evaluate(args):
    ...
    if args.eval_track is not None:
        ...
        with open(os.path.join(os.getenv("SCIHORIZON_ELAB_ROOT"), "configs/evaluation/tracks", f"{args.eval_track}.json"), "r") as f:
    ...
    if args.policy.lower() == "openvla":
        policy = OpenVLA(
            ...
            norm_config_file=os.path.join(os.getenv("SCIHORIZON_ELAB_ROOT"), "configs/model/openvla_config.json") # TODO: re-compuate the norm state by your own dataset
        )
    elif args.policy.lower() == "gr00t":
        from scihorizon_elab.benchmark.evaluation.model.policy.gr00t import Gr00tPolicy
        policy = Gr00tPolicy(host=args.host, port=args.port, replan_steps=args.replanstep)
    elif args.policy.lower() == "openpi":
        from scihorizon_elab.benchmark.evaluation.model.policy.openpi import OpenPiPolicy
        policy = OpenPiPolicy(host=args.host, port=args.port, replan_steps=args.replanstep)
    elif args.policy.lower() == "act":
        from scihorizon_elab.benchmark.evaluation.model.policy.act import ACTPolicy
        ...
        from scihorizon_elab.benchmark.evaluation.model.policy.dp import DPPolicy
        # Check if model_ckpt is provided and valid
        use_checkpoint = (hasattr(args, 'model_ckpt') and
                         args.model_ckpt and
                         args.model_ckpt != "none" and
