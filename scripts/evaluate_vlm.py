...
import json
import os
from scihorizon_elab.benchmark.evaluation.evaluator import VLMEvaluator
from scihorizon_elab.benchmark.evaluation.model.vlm import *
...
def main():
    ...
    for eval_dim in args.eval_dimension:
        if args.tasks is None:
            task_list = os.listdir(os.path.join(os.getenv("SCIHORIZON_ELAB_ROOT"), "../dataset", f"vlm_evaluation_v1.0/{eval_dim}"))
        else:
            task_list = args.tasks
        evaluator = VLMEvaluator(
            tasks=task_list, 
            n_episodes=args.n_episodes,
            data_path=os.path.join(os.getenv("SCIHORIZON_ELAB_ROOT"), "../dataset", f"vlm_evaluation_v1.0/{eval_dim}"),
            save_path=os.path.join(os.getenv("SCIHORIZON_ELAB_ROOT"), "../logs/vlm"),
        )
