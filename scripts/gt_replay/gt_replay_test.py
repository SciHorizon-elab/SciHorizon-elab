...
os.environ["SCIHORIZON_ELAB_ROOT"] = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
from scihorizon_elab.utils.utils import rotvec_to_quaternion, get_logger
...
def replay_trajectory(hdf5_path, save_video=False, save_dir=None):
    ...
    if task_name.endswith("_series"):
        ...
        _vlabench_root = os.environ.get("SCIHORIZON_ELAB_ROOT") or os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "scihorizon_elab"
        )
    ...
    for i, action in enumerate(trajectory):
        ...
        if should_print:
            print(f"  Step {i:3d}/{len(trajectory)}: pos=[{action[0]:.3f}, {action[1]:.3f}, {action[2]:.3f}], "
                  f"rotvec=[{action[3]:.3f}, {action[4]:.3f}, {action[5]:.3f}], "
        ...
        # 将 rotvec 转换为 quat 用于逆运动学
        pos = action[:3]
        rotvec = action[3:6]
        quat = rotvec_to_quaternion(rotvec)
...
def main():
    ...
    parser.add_argument("--save-dir", type=str, default="${SCIHORIZON_ELAB_DIR}/logs/gt_replay", help="Directory to save videos")
