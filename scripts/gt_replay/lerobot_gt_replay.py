...
sys.path.insert(0, "${SCIHORIZON_ELAB_DIR}")
...
from scihorizon_elab.utils.utils import rotvec_to_quaternion, quaternion_to_euler, get_logger
...
def lerobot_gt_replay(dataset_path, episode_idx=0, task=None, hdf5_path=None, save_video=False, save_dir=None):
    ...
    for i in range(num_frames):
        # 读 LeRobot action: [pos3_local, rotvec3_local, gripper_binary]
        action = actions[i]

        pos_local = action[:3]
        rotvec_local = action[3:6]
        ...
        if should_print:
            print(f"  Step {i:3d}/{num_frames}: local_pos=[{pos_local[0]:.3f}, {pos_local[1]:.3f}, {pos_local[2]:.3f}], "
                  f"rotvec=[{rotvec_local[0]:.3f}, {rotvec_local[1]:.3f}, {rotvec_local[2]:.3f}], "
        ...
        # 4. euler → quat
        quat = rotvec_to_quaternion(rotvec_local)
...
def main():
    ...
    parser.add_argument("--save-dir", type=str,
                        default="${SCIHORIZON_ELAB_DIR}/logs/lerobot_gt_replay",
