...
HDF5_DIR=${1:-"${SCIHORIZON_ELAB_DIR}/dataset/gt_test/lift_beaker"}
NUM_SAMPLES=${2:-5}

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCIHORIZON_ELAB_ROOT="$(dirname "$SCRIPT_DIR")"
...
for hdf5_file in "${HDF5_FILES[@]}"; do
    ...
    python "$SCRIPT_DIR/gt_replay_test.py" --hdf5-path "$hdf5_file" --vis --save-dir "$SCIHORIZON_ELAB_ROOT/gt_replay_results"
...
echo ""
echo "Videos saved to: $SCIHORIZON_ELAB_ROOT/gt_replay_results"
