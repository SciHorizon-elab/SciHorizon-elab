...
STORAGE_NAME="${SAVE_SUBDIR:-${SERIES_NAME}}"  # 存储目录名，默认=series名
PROJECT_ROOT=${SCIHORIZON_ELAB_ROOT:-$(cd "$(dirname "$0")/../.." && pwd)}
# SAVE_DIR 只到 series 这一层：trajectory_generation.py 内部会拼上 task_name
# (=SERIES_NAME)，最终落到 ${SERIES_NAME}/。convert_to_lerobot_pi.py 也是按
# series_name 作为 task-list 直接 os.walk 这一层，与下面 TASK_DIR 对齐。
# trajectory_generation.py 内部会用 task_name (=SERIES_NAME) 拼子目录:
# save_dir = args.save_dir, 最终存到 save_dir/place_beaker_X_series/
# 所以 SAVE_DIR 只到 training_data 这一层。
SAVE_DIR="$PROJECT_ROOT/dataset/training_data"
LOG_DIR="$PROJECT_ROOT/logs/traj_gen/${STORAGE_NAME}"
TASK_DIR="$PROJECT_ROOT/dataset/training_data/${STORAGE_NAME}"
...
while true; do
    ...
    for gpu in "${GPUS[@]}"; do
        ...
        CUDA_VISIBLE_DEVICES=$gpu \
            SCIHORIZON_ELAB_ROOT="$PROJECT_ROOT" \
            conda run -n vlabench_2 \
