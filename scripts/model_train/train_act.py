...
def main():
    ...
    print(f"[DEBUG] LEROBOT_ROOT: {LEROBOT_ROOT}", flush=True)
    print(f"[DEBUG] CONDA_SH: {CONDA_SH}", flush=True)
    print(f"[DEBUG] CONDA_SH exists: {os.path.exists(CONDA_SH)}", flush=True)
    log(f"  任务: {args.task}")
    log(f"  轨迹目标: {args.num}")
...
python lerobot/scripts/train.py \\
    ...
    > {train_log} 2>&1
        """)
        print(f"[DEBUG] Generated training command:\n{cmd}\n", flush=True)
