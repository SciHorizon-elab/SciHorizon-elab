...
用法:
    python scripts/model_train/train_pi.py --task pick_cylinder_mid_pour_object --num 200 --gpus 0,1,2,3,4,5,6 --train-steps 100000
"""
...
PROJECT_ROOT = os.environ.get("SCIHORIZON_ELAB_ROOT", os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
...
class TrajectoryGenMonitor(ProgressMonitor):
    """轨迹生成监控"""
    def __init__(self, task_name, save_subdir, num_target, interval=10):
    ...
    def _check(self):
        # 实际保存路径: dataset/training_data/<save_subdir>/<save_subdir>/
        # (trajectory_generation.py 用 --task-name=<save_subdir> 拼子目录)
        task_dir = f"{PROJECT_ROOT}/dataset/training_data/{self.save_subdir}"
...
def main():
    ...
    parser.add_argument("--batch-size", type=int, default=32, help="batch size（每卡。LoRA 下 A800 80GB 可承受 32，OOM 则降 16）")
    ...
    parser.add_argument("--save-subdir", type=str, default=None,
                        help="子目录名,默认 <task>_series.用于生成新轨迹集到指定目录而不混入旧数据")
    args = parser.parse_args()

    series_name = f"{args.task}_series"
    save_subdir = args.save_subdir or series_name  # e.g. "place_beaker_3_series_v2"
    lerobot_dataset = save_subdir
    ...
    if not args.skip_gen:
        ...
        cmd = conda_run("vlabench_2", f"bash {gen_script} --task {args.task} --num {args.num} --gpus {args.gpus} --samples {args.samples_per_gpu} --save-subdir {save_subdir}")

        # 后台运行生成
        proc = subprocess.Popen(cmd, shell=True, executable="/bin/bash", stdout=subprocess.PIPE, stderr=subprocess.STDOUT, bufsize=1)

        # 监控线程
        monitor = TrajectoryGenMonitor(args.task, save_subdir, args.num, interval=15)
        monitor.start()
...
python {PROJECT_ROOT}/scripts/convert_to_lerobot_pi.py \\
    ...
    if not args.skip_config:
        log("[3/5] 添加训练配置...")
        add_config(args.task, save_subdir, train_config_name, args.train_steps, args.batch_size)
    else:
...
def add_config(task_name, save_subdir, config_name, train_steps, batch_size):
...
'''
    ...
    # 添加任务注册
    init_file = f"{PROJECT_ROOT}/scihorizon_elab/configs/__init__.py"
    with open(init_file) as f:
        init_content = f.read()
    if f'"{save_subdir}"' in init_content:
        log(f"  任务注册已存在，跳过")
    else:
        marker2 = '    "place_beaker_on_the_mat_series":["place_beaker_on_the_mat"],'
        new_entry = f'    "{save_subdir}":["{task_name}"],\n{marker2}'
        init_content = init_content.replace(marker2, new_entry)
        with open(init_file, "w") as f:
            f.write(init_content)
        log(f"  已添加任务注册 {save_subdir}")
