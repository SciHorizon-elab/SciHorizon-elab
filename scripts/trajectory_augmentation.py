...
from scihorizon_elab.utils.utils import rotvec_to_quaternion
from scihorizon_elab.utils.camera_utils import translate_camera_keep_target, orbital_camera_movement
...
def get_args():
    ...
    parser.add_argument("--camera-aug-file", type=str, default="scihorizon_elab/configs/camera/front_camera_augmentation.json", help="Augmentation parameters for augmented cameras")
...
def augment_trajectory(episode_path, 
    ...
    if replay_mode == "joint":
        ...
        for action in actions:
            obs = env.get_observation()
            observations.append(obs)
            point, rotvec, gripper_state = action[:3], action[3:6], action[-2:]
