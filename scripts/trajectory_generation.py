...
from scihorizon_elab.simulation.robots import Franka
from scihorizon_elab.benchmark.tasks import *
from scihorizon_elab.utils.data_utils import save_single_data, process_observations
from scihorizon_elab.utils.utils import find_key_by_value, get_logger
...
def get_args():
    ...
    parser.add_argument('--save-dir', default="/media/shiduo/LENOVO_USB_HDD/dataset/scihorizon_elab")
    ...
    parser.add_argument('--storage-name', default=None, type=str,
                        help='storage subdirectory name (default: task_name). Use this to save data to a different dir than task_name.')
    args = parser.parse_args()
    # Default storage_name to task_name if not specified
    if args.storage_name is None:
        args.storage_name = args.task_name
    return args
...
def generate_trajectory(args, index, logger):
    ...
    # task_dir 使用 args.save_dir，对于 _series 任务自动拼子目录，
    # 这样 generate_trajectories.sh 只需传基础 training_data 目录即可。
    # 优先使用 --storage-name 指定目录，忽略 task_name 的 _series 检测。
    task_dir = args.save_dir
    if args.storage_name:
        task_dir = os.path.join(args.save_dir, args.storage_name)
...
if __name__ == "__main__":
    ...
    if args.task_name.endswith("_series"):
        ...
        # repo 根目录 = scripts/ 的上一级（不要硬编码 /ssd/...，换机器会失效）
        _vlabench_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _series_path = os.path.join(
            _vlabench_root, "scihorizon_elab", "benchmark", "tasks", "autogen_tasks", "primitive", f"{args.task_name}.py"
        )
        if not os.path.exists(_series_path):
            _series_path = os.path.join(
                _vlabench_root, "scihorizon_elab", "benchmark", "tasks", "autogen_tasks", f"{args.task_name}.py"
            )
        if not os.path.exists(_series_path):
            # 与 run_task.py 一致：深度扫描 task_list/ 子树
            _candidates = sorted(_glob.glob(os.path.join(
                _vlabench_root, "scihorizon_elab", "benchmark", "tasks", "task_list", "**", f"{args.task_name}.py"
            ), recursive=True))
            if _candidates:
                _series_path = _candidates[0]
        if os.path.exists(_series_path):
            ...
            from scihorizon_elab.benchmark.configs import name2config as _n2c
            from scihorizon_elab.utils.register import register as _register
    ...
    logger = get_logger()
    # 计算任务子目录，与 generate_trajectory 中保持一致
    # 优先使用 --storage-name 指定目录，忽略 task_name 的 _series 检测
    task_dir = args.save_dir
    if args.storage_name:
        task_dir = os.path.join(args.save_dir, args.storage_name)
    for i in tqdm(range(args.n_sample)):
        i += args.start_id
        try:
            h5_files = get_all_hdf5_files(task_dir)
